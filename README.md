...
The root `main.py` is intentionally thin. The active member is controlled by `CURRENT_MEMBER` in `prosperity/strategies/trader.py`. The file you upload to IMC should be the exported single-file submission generated by `scripts/shared/export_submission.py`.
...
```powershell
python scripts/shared/analyze_log.py --log examples\official_logs\16248.log --outdir artifacts\analysis
```
...
```powershell
python scripts/shared/benchmark_strategy.py --strategy champion --day -2
```
...
- `scripts/shared/export_submission.py`: single-file exporter
