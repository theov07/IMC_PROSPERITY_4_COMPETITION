"""Trend carry market maker V34.

V34 keeps the V29 sell/trim framework, but borrows the V18 entry style:

- sells still come from the V29 carry + trim engine
- re-entries are anchored to a colder FV-style reference
- after trims, we keep a lower V18-like anchor bid alive to catch dips
"""
...
class TrendCarryMMV34Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        if anchor_mode and buy_cap > buy_size:
            ...
            if candidate_anchor_bid >= 1:
                ...
                anchor_buy_size = min(max(1, v18_anchor_buy_size), max(0, buy_cap - buy_size))
