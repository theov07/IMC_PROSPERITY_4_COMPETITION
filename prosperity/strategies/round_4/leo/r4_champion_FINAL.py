...
class HydroMVV6(BaseStrategy):
    def _update_anchor(self, raw_mid, position, memory):
        ...
        if mode == 'fixed':
            ...
            if drift_bound > 0:
                anchor_ema = max(anchor_fixed - drift_bound, min(anchor_fixed + drift_bound, anchor_ema))
        elif mode == 'slow_ewma':
            if anchor_alpha > 0:
                anchor_ema = anchor_alpha * raw_mid + (1.0 - anchor_alpha) * anchor_ema
        ...
        return anchor_ema
    def _update_ar(self, raw_mid, position, memory):
    ...
    def _update_m14(self, state, memory):
        ...
        return signal
    def _passive_sizes(self, position):
        limit = self.position_limit()
    ...
    def _passive_quotes(self, book, bid_size, ask_size, buy_cap, sell_cap, position, dev):
        ...
        if self.params.get('use_ar_quote_bias', False) and bid_price and ask_price:
            ...
            if dev > 0:
                ask_price = max(book.best_bid + 1 if book.best_bid else ask_price, ask_price - bias_ticks)
            elif dev < 0:
                bid_price = min(book.best_ask - 1 if book.best_ask else bid_price, bid_price + bias_ticks)
        ...
        return orders
    def _ar_takers(self, book, order_depth, fair_value, dev, bid_size, ask_size, buy_cap, sell_cap):
    ...
    def _guard_allows_taker(self, raw_mid, position, memory):
    ...
    def compute_orders(self, state, book, order_depth, position, memory):
        ...
        taker_orders, buy_cap, sell_cap, taker_buy_px, taker_sell_px = self._ar_takers(book, order_depth, fair_value, dev, bid_size, ask_size, buy_cap, sell_cap) if guard_ok else ([], buy_cap, sell_cap, set(), set())
        passive_orders = self._passive_quotes(book, bid_size, ask_size, buy_cap, sell_cap, position, dev)
        ...
        self.log_quote_snapshot(state=state, memory=memory, bid_price=None, ask_price=None, extras={'position': position, 'mid': round(float(mid), 2), 'FairValue': round(fair_value, 2), 'Anchor': round(anchor_val, 2), 'DevSmooth': round(dev, 3), 'ar_mom': round(float(memory.get('_ar_momentum', 0.0)), 4), 'guard': int(guard_ok), 'M14Signal': signal, 'taker_sell': taker_sold, 'taker_buy': taker_bought, 'bid_size': int(bid_size), 'ask_size': int(ask_size), 'sigma': round(sigma, 4)})
        return (all_orders, 0)
    def feature_prices(self, memory):
        out: Dict[str, float] = {}
        ...
        if (v := memory.get('_guard_on')) is not None:
            out['guard'] = float(v)
        return out
_VB = {'counterparty_bias_enabled': False, 'enable_takers': False, 'inv_bias_per_unit': 0.02, 'inv_unwind_enabled': True, 'inv_unwind_max_per_tick': 10, 'inv_unwind_mode': 'passive', 'inv_unwind_passive_offset': -1, 'inv_unwind_passive_size': 30, 'inv_unwind_target_pct': 0.5, 'inv_unwind_threshold_pct': 0.7, 'last_ts_value': 999900, 'log_flush_ts': 1000, 'maker_edge': 2, 'maker_size': 20, 'min_quote_price': 2.0, 'obi_size_enabled': False, 'penny_improve_around_mkt': True, 'position_limit': 300, 'prior_vol': 0.0125, 'sigma_cap': 0.1, 'sigma_floor': 0.005, 'take_edge': 3.0, 'take_size': 40, 'timestamp_units_per_day': 1000000, 'ts_increment': 100, 'tte_days_initial': 4.0, 'underlying_symbol': 'VELVETFRUIT_EXTRACT', 'use_smile': True, 'vol_size_cut_enabled': True, 'vol_size_cut_factor': 0.5, 'vol_size_cut_threshold': 0.008, 'vol_size_cut_window': 50}
PRODUCTS = {
    'HYDROGEL_PACK': {'anchor_price': 10000, 'anchor_alpha': 0.005, 'anchor_drift_bound': 1.5, 'ar_gain': 8.0, 'ar_smooth_half_life': 5, 'mid_smooth_half_life': 20, 'dev_smooth_half_life': 5, 'passive_quoting': True, 'maker_size_base_pct': 0.25, 'pct_kept_for_takers': 0.2, 'use_inventory_bias': True, 'use_ar_taker': True, 'ar_taker_edge': 12.0, 'ar_taker_size_pct': 0.3, 'use_gap_exploit': False, 'use_m14_gate': False, 'use_ar_quote_bias': False, 'use_anchor_guard': False, 'informed_trader_name': 'Mark 14', 'last_ts_value': 999900, 'quote_trace_enabled': True, 'log_flush_ts': 1000, 'anchor_mode': 'inv_protected', 'anchor_pos_threshold': 0.2, 'strategy': 'hydro_mv_v6', 'position_limit': 200},
    'VELVETFRUIT_EXTRACT': {'anchor_alpha': 0.02, 'anchor_drift_bound': 2.0, 'anchor_price': 5250.0, 'ar_gain': 0.3, 'ar_shift_source': 'mid_smooth', 'counterparty_bias_enabled': True, 'cp_anchor_scale_per_unit': 0.15, 'cp_conditional_baseline_weight': 0.0, 'cp_conditional_min_samples': 50, 'cp_conditional_stats_window_ts': 50000, 'cp_conditional_traders': ['Mark 22'], 'cp_conditional_zthresh': 1.5, 'cp_max_anchor_offset': 2.0, 'cp_signal_threshold': 1.0, 'cp_trader_weights': {'Mark 01': -0.2, 'Mark 14': -0.5, 'Mark 22': -0.4, 'Mark 49': -0.8}, 'cp_window_ts': 10000, 'full_capacity_on_empty': True, 'guard_inventory_dist': 40.0, 'guard_max_dist': 80.0, 'guard_min_dist': 0.0, 'guard_near_band': 0.0, 'guard_reversion_threshold': 7.5, 'guard_trend_alpha': 0.45, 'inventory_aversion_gamma': 0.001, 'last_ts_value': 999900, 'log_flush_ts': 1000, 'maker_size': 30, 'maker_size_base_pct': 0.4, 'obi_size_boost_factor': 1.5, 'obi_size_enabled': True, 'obi_size_levels': 3, 'obi_size_reduce_factor': 0.7, 'obi_size_threshold': 0.005, 'passive_unwind_skew_ticks': 1, 'passive_unwind_trigger': 0.38, 'pct_kept_for_takers': 0.005, 'position_limit': 200, 'strategy': 'r3_guarded_anchor_mm', 'take_edge_hi': 1.2, 'take_edge_lo': 0.6, 'tighten_ticks': 1, 'toxic_size_frac': 0.68, 'toxic_threshold': 0.6, 'toxic_window': 8, 'ts_increment': 100, 'unwind_take_edge': 3.0},
    'VEV_4000': {**_VB, 'boost_when_cheap': False, 'edge_ticks': 0.0, 'entry_size': 30, 'entry_size_boost': 1.5, 'implied_vol_prior': 0.0125, 'passive_bid_size': 24, 'skip_when_expensive': True, 'strategy': 'gamma_scalp_zgated', 'strike': 4000, 'target_qty': 300, 'unwind_tte_threshold': 1.5},
    'VEV_4500': {**_VB, 'boost_when_cheap': False, 'edge_ticks': 0.0, 'entry_size': 30, 'implied_vol_prior': 0.0125, 'iv_boost_threshold': 0.001, 'iv_delta_threshold': 0.0003, 'iv_ewma_alpha': 0.3, 'iv_ewma_fast_alpha': 0.1, 'iv_ewma_slow_alpha': 0.02, 'iv_passive_boost': 1.5, 'iv_residual_gate': True, 'iv_skip_threshold': 0.001, 'passive_bid_size': 24, 'skip_when_expensive': True, 'strategy': 'gamma_scalp_zgated', 'strike': 4500, 'target_qty': 300, 'unwind_tte_threshold': 1.5},
    'VEV_5000': {**_VB, 'boost_when_cheap': False, 'edge_ticks': 0.0, 'entry_size': 30, 'implied_vol_prior': 0.0125, 'iv_boost_threshold': 0.001, 'iv_delta_threshold': 0.0003, 'iv_ewma_alpha': 0.3, 'iv_ewma_fast_alpha': 0.1, 'iv_ewma_slow_alpha': 0.02, 'iv_passive_boost': 1.5, 'iv_residual_gate': True, 'iv_skip_threshold': 0.001, 'passive_bid_size': 24, 'skip_when_expensive': True, 'strategy': 'gamma_scalp_zgated', 'strike': 5000, 'target_qty': 300, 'unwind_tte_threshold': 1.5},
    'VEV_5100': {**_VB, 'boost_when_cheap': False, 'edge_ticks': 0.0, 'entry_size': 30, 'implied_vol_prior': 0.0125, 'iv_boost_threshold': 0.001, 'iv_delta_threshold': 0.0003, 'iv_ewma_alpha': 0.3, 'iv_ewma_fast_alpha': 0.1, 'iv_ewma_slow_alpha': 0.02, 'iv_passive_boost': 1.5, 'iv_residual_gate': True, 'iv_skip_threshold': 0.001, 'passive_bid_size': 24, 'skip_when_expensive': True, 'strategy': 'gamma_scalp_zgated', 'strike': 5100, 'target_qty': 300, 'unwind_tte_threshold': 1.5},
    'VEV_5200': {**_VB, 'ask_offset_neutral': 10, 'ask_offset_sell': 1, 'cross_asset_enabled': True, 'cross_asset_max_offset': 2.0, 'cross_asset_scale': 0.1, 'cross_asset_source_symbol': 'VELVETFRUIT_EXTRACT', 'cross_asset_source_trader': 'Mark 14', 'cross_asset_threshold': 5.0, 'cross_asset_weight': -0.5, 'cross_asset_window_ts': 10000, 'delta_sigma': 0.022, 'maker_size_ask': 5, 'maker_size_bid': 20, 'prevent_crossing': False, 'strategy': 'vev_option_mm_v3', 'strike': 5200.0, 'zscore_exec_mode': 'none'},
    'VEV_5300': {**_VB, 'boost_when_cheap': False, 'edge_ticks': 0.0, 'entry_size': 30, 'implied_vol_prior': 0.0125, 'iv_boost_threshold': 0.001, 'iv_delta_threshold': 0.0003, 'iv_ewma_alpha': 0.3, 'iv_ewma_fast_alpha': 0.1, 'iv_ewma_slow_alpha': 0.02, 'iv_passive_boost': 1.5, 'iv_residual_gate': True, 'iv_skip_threshold': 0.001, 'passive_bid_size': 24, 'skip_when_expensive': True, 'strategy': 'gamma_scalp_zgated', 'strike': 5300, 'target_qty': 300, 'unwind_tte_threshold': 1.5},
    'VEV_5400': {**_VB, 'ask_offset_neutral': 10, 'ask_offset_sell': 1, 'delta_sigma': 0.022, 'maker_size_ask': 5, 'maker_size_bid': 20, 'prevent_crossing': True, 'strategy': 'vev_option_mm_v3', 'strike': 5400.0, 'zscore_exec_mode': 'none'},
}
STRATEGY_CLASSES = {'gamma_scalp_zgated': GammaScalpZGatedStrategy, 'r3_guarded_anchor_mm': R3GuardedAnchorMMStrategy, 'vev_option_mm_v3': VEVOptionMMV3Strategy, 'hydro_mv_v6': HydroMVV6}
class Trader:
    """Hybrid Trader: v24 MM on VELVET + TraderBSV mean reversion on HYDROGEL/vouchers."""
    POSITION_LIMITS = {'VELVETFRUIT_EXTRACT': 200, 'HYDROGEL_PACK': 200, 'VEV_4000': 300, 'VEV_4500': 300, 'VEV_5000': 300, 'VEV_5100': 300, 'VEV_5200': 300, 'VEV_5300': 300, 'VEV_5400': 300, 'VEV_5500': 300, 'VEV_6000': 300, 'VEV_6500': 300}
    REFERENCE_S = 5250.0
    REFERENCE_H = 10000.0
    EMA_HL = 25
    HYD_MR_THR, HYD_MR_EXIT, HYD_MR_SIZE = (15.0, 3.0, 200)
    VOUCHER_MR_KS = [5000, 5100, 5200, 5300, 5400, 5500]
    VOUCHER_MR_THR, VOUCHER_MR_EXIT, VOUCHER_MR_SIZE = (7.0, 1.0, 300)
    VEV4500_THR, VEV4500_EXIT, VEV4500_SIZE = (18.0, 1.0, 300)
    VEV4000_THR, VEV4000_EXIT, VEV4000_SIZE = (35.0, 3.0, 300)
    def __init__(self):
        vc = PRODUCTS['VELVETFRUIT_EXTRACT']
        self.velvet_strategy = STRATEGY_CLASSES[vc['strategy']](product='VELVETFRUIT_EXTRACT', params={k: v for k, v in vc.items() if k != 'strategy'})
        hc = PRODUCTS['HYDROGEL_PACK']
        self.hydro_strategy = STRATEGY_CLASSES[hc['strategy']](product='HYDROGEL_PACK', params={k: v for k, v in hc.items() if k != 'strategy'})
    def bid(self):
        return 15
    @staticmethod
    def _ema_alpha(half_life):
    ...
    def _bid_ask(od):
        ...
        return (bid, ask)
    def _signal(self, ema, ref, thr, exit_thr, size, cur):
        dev = ema - ref
        if dev < -thr:
            return +size
        if dev > +thr:
        ...
        if abs(dev) < exit_thr:
            return 0
        return cur
    def _orders_to_target(self, product, current, target, bid, ask):
        ...
        if bid is None:
            return []
        return [Order(product, int(bid), int(diff))]
    def run(self, state):
        ...
        if 'VELVETFRUIT_EXTRACT' in state.order_depths:
            mem = product_memories.setdefault('VELVETFRUIT_EXTRACT', {})
            mem['_shared'] = {'timestamp': state.timestamp}
            orders, convs = self.velvet_strategy.on_tick(state, mem)
        ...
        bsv = saved.setdefault('bsv', {})
        if 'ema_S' not in bsv:
            bsv['ema_S'] = self.REFERENCE_S
        if 'ema_H' not in bsv:
            bsv['ema_H'] = self.REFERENCE_H
        a_ema = self._ema_alpha(self.EMA_HL)
        if S_mid is not None:
            bsv['ema_S'] = a_ema * S_mid + (1.0 - a_ema) * bsv['ema_S']
        if H_mid is not None:
            bsv['ema_H'] = a_ema * H_mid + (1.0 - a_ema) * bsv['ema_H']
        ema_S = bsv['ema_S']
        ema_H = bsv['ema_H']
        positions = state.position
        if 'HYDROGEL_PACK' in ods:
            mem_h = product_memories.setdefault('HYDROGEL_PACK', {})
            mem_h['_shared'] = {'timestamp': state.timestamp}
            orders_h, convs_h = self.hydro_strategy.on_tick(state, mem_h)
            mem_h.pop('_shared', None)
            if orders_h:
                result['HYDROGEL_PACK'] = orders_h
            total_conversions += convs_h
        for K in self.VOUCHER_MR_KS:
            sym = f'VEV_{K}'
            if sym not in ods:
                continue
            cur = positions.get(sym, 0)
            target = self._signal(ema_S, self.REFERENCE_S, self.VOUCHER_MR_THR, self.VOUCHER_MR_EXIT, self.VOUCHER_MR_SIZE, cur)
            bid, ask = self._bid_ask(ods[sym])
            orders = self._orders_to_target(sym, cur, target, bid, ask)
            if orders:
                result[sym] = orders
        if 'VEV_4500' in ods:
            cur = positions.get('VEV_4500', 0)
            target = self._signal(ema_S, self.REFERENCE_S, self.VEV4500_THR, self.VEV4500_EXIT, self.VEV4500_SIZE, cur)
            bid, ask = self._bid_ask(ods['VEV_4500'])
            orders = self._orders_to_target('VEV_4500', cur, target, bid, ask)
            if orders:
                result['VEV_4500'] = orders
        if 'VEV_4000' in ods:
            cur = positions.get('VEV_4000', 0)
            target = self._signal(ema_S, self.REFERENCE_S, self.VEV4000_THR, self.VEV4000_EXIT, self.VEV4000_SIZE, cur)
            bid, ask = self._bid_ask(ods['VEV_4000'])
            orders = self._orders_to_target('VEV_4000', cur, target, bid, ask)
            if orders:
