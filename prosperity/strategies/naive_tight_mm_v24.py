"""Naive passive market maker V24.

V24 stays close to V17 but applies the inventory-band policy discussed after
reviewing the official runs:

- 0-60: normal long-biased accumulation
- 60-75: assisted accumulation, still very reluctant to sell
- 75-80: top-up the last lots, trims only in tiny clips above a floor
- once trimmed below the floor, the strategy naturally goes back to reacquire

The goal is to keep the strong carry profile of V17 while improving how quickly
the strategy reaches and re-reaches the last 5-20 lots of inventory.
"""
...
class NaiveTightMarketMakerV24Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        rebuy_block_position_floor = int(self.params.get("rebuy_block_position_floor", 75))
        ...
        trim_floor_position = int(self.params.get("trim_floor_position", 75))
        trim_signal_edge = float(self.params.get("trim_signal_edge", 1.0))
        trim_cooldown_ticks = int(self.params.get("trim_cooldown_ticks", 8))

        accum_band_trend_threshold = float(self.params.get("accum_band_trend_threshold", 4.0))
        accum_mid_start_position = int(self.params.get("accum_mid_start_position", 60))
        accum_top_start_position = int(self.params.get("accum_top_start_position", 75))
        accum_mid_take_boost = float(self.params.get("accum_mid_take_boost", 0.25))
        accum_mid_buy_size_boost = float(self.params.get("accum_mid_buy_size_boost", 0.25))
        accum_mid_take_cap = int(self.params.get("accum_mid_take_cap", 8))
        accum_top_take_boost = float(self.params.get("accum_top_take_boost", 0.10))
        accum_top_buy_size_boost = float(self.params.get("accum_top_buy_size_boost", 0.10))
        accum_top_take_cap = int(self.params.get("accum_top_take_cap", 8))
        ...
        strong_trend = signal_mode == "trend" and trend_shift >= accum_band_trend_threshold
        accum_mid_band = strong_trend and accum_mid_start_position <= position < accum_top_start_position
        accum_top_band = strong_trend and accum_top_start_position <= position < limit

        core_position = int(round(limit * core_position_frac))
        above_core = position >= core_position
        block_rebuy = (
            position >= rebuy_block_position_floor
            and not on_dip
            and extension >= rebuy_block_extension_threshold
        )
        ...
        if accum_mid_band:
            buy_edge = max(0.0, buy_edge - accum_mid_take_boost)
        elif accum_top_band:
            buy_edge = max(0.0, buy_edge - accum_top_take_boost)
        ...
        if accum_mid_band:
            take_buy_cap = max(take_buy_cap, accum_mid_take_cap)
        elif accum_top_band:
            take_buy_cap = max(take_buy_cap, accum_top_take_cap)
        ...
        if accum_mid_band and buy_size > 0:
            buy_size = min(buy_cap, max(1, int(round(buy_size * (1.0 + accum_mid_buy_size_boost)))))
        elif accum_top_band and buy_size > 0:
            buy_size = min(buy_cap, max(1, int(round(buy_size * (1.0 + accum_top_buy_size_boost)))))
        ...
        trim_signal = strong_trend and book.best_bid >= fast_ema + trim_signal_edge
        last_trim_ts = int(memory.get("last_trim_ts", -10**9))
        trim_mode = (
            position > trim_floor_position
            and not on_dip
            and trend_shift >= trim_trend_threshold
            and extension >= trim_extension_threshold
            and trim_signal
            and state.timestamp - last_trim_ts >= trim_cooldown_ticks * 100
        )

        allowed_sell = max(0, position - trim_floor_position) if strong_trend else sell_cap
        if strong_trend and position <= trim_floor_position:
            sell_size = 0

        if trim_mode and sell_cap > 0 and allowed_sell > 0:
            ...
            if position <= trim_floor_position:
                sell_size = 0
            elif trend_hold_sell_size_frac <= 0.0:
                sell_size = 0
            else:
                sell_size = int(round(sell_size * trend_hold_sell_size_frac))
                sell_size = min(sell_cap, allowed_sell, max(1, sell_size))

        if strong_trend and sell_size > 0:
            sell_size = min(sell_size, allowed_sell)
        ...
        memory["accum_mid_band"] = accum_mid_band
        memory["accum_top_band"] = accum_top_band
        ...
        self.log_quote_snapshot(
            ...
            extras={
                ...
                "acc_mid": int(accum_mid_band),
                "acc_top": int(accum_top_band),
            },
        )
