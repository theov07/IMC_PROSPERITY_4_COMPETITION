"""Trend carry market maker V37.

V37 keeps the V34 carry + trim framework, but changes the startup inventory path:

- fill the first inventory block quickly while the trend is young
- once the first block is built, switch to a colder entry reference
- keep a lower anchor bid alive to finish the last lots more cleanly

Goal: reduce the early mark-to-market drawdown without giving up the long carry.
"""
...
class TrendCarryMMV37Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        startup_fast_target = int(self.params.get("startup_fast_target", 52))
        startup_cold_take_cap = int(self.params.get("startup_cold_take_cap", 4))
        startup_cold_take_edge_relax = float(self.params.get("startup_cold_take_edge_relax", 2.5))
        startup_cold_buy_frac = float(self.params.get("startup_cold_buy_frac", 0.55))
        startup_cold_join_ticks = int(self.params.get("startup_cold_join_ticks", 0))
        startup_cold_anchor_size = int(self.params.get("startup_cold_anchor_size", 4))
        ...
        take_reference = fair
        if bullish and not on_dip and (position >= trim_floor_position or startup_cold_loading):
        ...
        if startup_cold_loading and buy_size > 0:
        ...
        if bullish and buy_size > 0:
            join_ticks = startup_cold_join_ticks if startup_cold_loading else bid_join_ticks
        ...
        anchor_buy_size = 0
        anchor_mode = bullish and not on_dip and (rebuy_blocked or trim_quote_mode or chasing or startup_cold_loading)
