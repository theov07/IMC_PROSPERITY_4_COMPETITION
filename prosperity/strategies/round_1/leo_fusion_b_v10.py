"""Fusion B v10 — fast-fill with early deep-sweep guard.

Same goal as v9:
  - reach +80 inventory quickly in the persistent up-drift
  - stay long-biased and avoid selling before the core carry is built

Extra rule:
  - during the very early build phase, do not sweep into deeper ask levels
    when they are materially more expensive than the best ask in the same
    snapshot; wait for the next ticks instead.
"""
...
class LeoFusionBV10Strategy(Round1RegressionMMV5Strategy):
    def compute_orders(
        self,
        state: TradingState,
        book: BookSnapshot,
        ...
        self.log_quote_snapshot(
            ...
            extras={
                ...
                "buy_size": buy_size,
                "sell_size": sell_size,
                "deep_take_guard": int(deep_take_guard),
                "first_take_ask": first_take_ask if first_take_ask is not None else -1,
                "deep_take_blocked": int(deep_take_blocked),
            },
        )
        return orders, 0
