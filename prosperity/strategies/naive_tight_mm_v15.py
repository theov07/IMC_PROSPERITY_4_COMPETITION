"""Naive passive market maker V15.

V15 is a conservative follow-up to V11:

1. Keep the V11 long-hold behavior for INTARIAN_PEPPER_ROOT.
2. Re-enable tiny passive asks only when we are already very long and the
   positive trend is strong.
3. Push those asks further away so we only trim on strength, rather than
   reintroducing the V10 churn pattern.

This lets us test the user's concern ("from V11 onward there are no sells")
without switching to a fully active inventory cycling strategy.
"""
...
class NaiveTightMarketMakerV15Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        trend_hold_ask_extra_ticks = int(self.params.get("trend_hold_ask_extra_ticks", 0))
        ...
        if hold_long_trend:
            ask_price += trend_hold_ask_extra_ticks
            if sell_size > 0:
