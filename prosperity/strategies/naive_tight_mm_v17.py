"""Naive passive market maker V17.

V17 extends V16 with an explicit trim regime for IPR:

- keep the smaller, anti-chase entry clips from V16
- once we already hold a substantial long core, stop re-buying blindly
- when trend is strong and price is locally extended, quote a small ask to trim

This is the first version that intentionally tries to produce IPR sells instead
of hoping they appear as a side-effect of the hold logic.
"""
...
class NaiveTightMarketMakerV17Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        block_rebuy = above_core and not on_dip and extension >= rebuy_block_extension_threshold
        trim_mode = above_core and not on_dip and trend_shift >= trim_trend_threshold and extension >= trim_extension_threshold
        ...
        if trim_mode and sell_cap > 0:
            sell_size = min(sell_cap, max(1, trim_sell_size))
