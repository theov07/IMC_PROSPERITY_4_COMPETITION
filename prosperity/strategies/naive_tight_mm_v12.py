"""Naive passive market maker V12.

V12 extends V11 with one additional IPR-focused feature:

7. Trend hold mode (new params: ``trend_hold_threshold``,
   ``trend_hold_min_position_frac``, ``trend_hold_sell_size_frac``):
   In the official IPR logs, V10 often sells passively in a strong uptrend and
   then re-buys the same inventory one or two ticks later by crossing the ask.
   This creates churn and gives back edge. When the trend signal is strong and
   we are already well long, V11 can suppress or shrink passive asks so we keep
   the long inventory instead of recycling it immediately.

8. Trend uptick chase block (new params: ``trend_buy_uptick_block_threshold``,
   ``trend_buy_uptick_min_position_frac``):
   In V11, the remaining IPR edge problem is the initial accumulation phase:
   the strategy still crosses the ask right after some upward ask steps.
   V12 skips aggressive buy-taking for one tick when the best ask has just
   moved higher in a strong uptrend and we already have some long inventory.
   This aims to buy the follow-up pause / pullback instead of the first chase.

New params:
  trend_hold_threshold         : minimum positive trend_shift to enter hold mode
  trend_hold_min_position_frac : minimum long inventory fraction of the limit
                                 before asks are suppressed
  trend_hold_sell_size_frac    : passive ask size multiplier while hold mode is
                                 active (0.0 = no asks)
  trend_buy_uptick_block_threshold : minimum positive trend_shift to block
                                     aggressive buys on an ask uptick
  trend_buy_uptick_min_position_frac : minimum long inventory fraction before
                                       the uptick buy block is armed
"""
...
class NaiveTightMarketMakerV12Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        trend_buy_uptick_block_threshold = float(self.params.get("trend_buy_uptick_block_threshold", 0.0))
        trend_buy_uptick_min_position_frac = float(self.params.get("trend_buy_uptick_min_position_frac", 1.01))
        ...
        # If the ask just stepped higher in a strong uptrend, skip crossing the
        # first uptick once we already hold some inventory.
        ask_rose = bool(prev_best_ask is not None and book.best_ask is not None and book.best_ask > prev_best_ask)
        block_uptick_buy_take = (
            signal_mode == "trend"
            and ask_rose
            and trend_shift >= trend_buy_uptick_block_threshold > 0.0
            and position >= int(round(limit * trend_buy_uptick_min_position_frac))
        )
        ...
        # Strong-uptrend hold mode:
        # if we are already heavily long, avoid passively offering inventory
        # that we are likely to re-buy moments later at the ask.
        hold_long_trend = (
            signal_mode == "trend"
        ...
        memory["block_uptick_buy_take"] = block_uptick_buy_take
        ...
        self.log_quote_snapshot(
            ...
            extras={
                ...
                "block_uptick_buy_take": int(block_uptick_buy_take),
            },
        )
