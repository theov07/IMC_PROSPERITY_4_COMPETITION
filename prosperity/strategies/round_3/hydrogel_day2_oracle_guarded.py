"""HYDRO day2 oracle + guarded — slim version (no anchor fallback).

Stripped from hydrogel_day2_selector_mm to reduce inlined submission size.
Used by r3_hydro_day2_oracle_regime.

Logic:
  - day2 fingerprint detected → use L1 oracle replay
  - otherwise → use guarded Theo profile

Skips the anchor v4 profile entirely (saves ~50 KB on inlined submission).
"""
