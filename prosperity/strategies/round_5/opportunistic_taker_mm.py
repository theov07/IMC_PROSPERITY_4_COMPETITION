"""Opportunistic Taker MM — passive base + taker overlay on fair-anchor divergence.

Concept (ported from teammate's best_r5_mm_taker_v3):
  - Base passive MM (mode=naive / pair / coint / carry)
  - Computes a `fair_anchor = mid + signal_ticks + micro_ticks + trend_ticks`
  - Fires taker orders when ask <= fair_anchor - edge OR bid >= fair_anchor + edge
  - Inventory-aware unwind, taker cooldown (delta-based, NOT timestamp-anchored)
  - Optional inventory size damping

NOT time-based overfit: only uses state.timestamp for taker cooldown DELTA
(taker_cooldown_ts ticks between successive takers), not for any session-anchored
signal.

Per-product tuning (acceptable per user — not time-based):
  mode                       'naive' / 'pair' / 'coint' / 'carry'
  partner / partner_sign     for mode='pair'
  pair_thresh / z_window     pair signal params
  mean_half_life             for mode='coint' (EWMA spread mean)
  microprice_weight/clamp    microprice deviation bias
  trend_weight/clamp/hl      EMA trend bias
  signal_shift_per_unit/clamp  shift base from signal value
  opportunity_taker_size     taker order size
  taker_threshold            edge in ticks for taker firing
  min_opportunity_ticks      min |fair-mid| to allow taker
  taker_position_cap         max position taker can build
  taker_cooldown_ts          min delta between takers (DELTA, not timestamp)
  inv_skew_thresh/ticks      inventory-driven quote skew
  size_inv_factor            shrink quote size per |position|/limit
  unwind_min_pos/size/edge   inventory unwind via taker
  carry_pause_min_pos        for mode='carry'
"""
...
class OpportunisticTakerMMStrategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        # Coint mode: explicit taker entries on z-score
        if mode == "coint":
            ...
            if position == 0:
                if coint_z > entry_z and sell_room > 0:
                    ...
                    if qty > 0:
                        orders.append(Order(self.product, int(book.best_ask), qty))
                        buy_room -= qty
        # Opportunistic taker overlay (delta-based cooldown — NOT time-based overfit)
        ...
        if not did_taker and abs(position) >= unwind_min_pos:
            if position > 0 and sell_room > 0 and book.best_bid >= fair_anchor + unwind_edge:
                ...
                if qty > 0:
                    orders.append(Order(self.product, int(book.best_ask), qty))
                    buy_room -= qty
        # Inventory-aware passive sizing
