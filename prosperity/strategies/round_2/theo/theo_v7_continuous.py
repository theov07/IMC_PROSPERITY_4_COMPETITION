"""Theo v7 continuous strategy — standing deep orders + gap exploit.

Extracted from live-tested v7_continuous submission (PnL record 9,275 on IPR).
Chain: V1 → V2 → V3 → V4 → V5 → V6 → V7 (continuous standing deeps).
"""
...
class TheoBestCleanGeneralizedV7Strategy(TheoBestCleanGeneralizedV6Strategy):
    ...
    def compute_orders(
        ...
        # HARD CAP on normal MM buys: position + sum(buys) <= normal_mm_buy_cap
        # Only standing_deep (gap exploit baisse) is allowed to push past this cap.
        ...
        if mm_buy_cap < limit:
            ...
            if excess > 0:
                # Trim from lowest-price buys first (least aggressive) until excess = 0
                buys = sorted(
        ...
        # Compute capacity already consumed by super() (after cap applied)
        total_buy = sum(o.quantity for o in orders if o.quantity > 0)
