"""Black-Scholes pricing (European options) with pure-Python math.

All quantities in consistent daily units:
  S      : spot price of underlying
  K      : strike
  T      : time to expiry in DAYS (use float; fractional days OK)
  sigma  : daily volatility (std of log-returns per day)
  r      : risk-free rate per day (default 0 — prosperity assumption)
  q      : dividend yield per day (default 0)

Mathematical notes:
  With daily units the BS formula is:
    d1 = (ln(S/K) + (r - q + sigma^2/2) * T) / (sigma * sqrt(T))
    d2 = d1 - sigma * sqrt(T)
  Call  = S * e^(-q*T) * N(d1) - K * e^(-r*T) * N(d2)
  Delta = e^(-q*T) * N(d1)
  Gamma = e^(-q*T) * n(d1) / (S * sigma * sqrt(T))
  Vega  = S * e^(-q*T) * n(d1) * sqrt(T)   [per unit of sigma]
  Theta = -S * n(d1) * sigma * e^(-q*T) / (2 sqrt(T))
          - r * K * e^(-r*T) * N(d2) + q * S * e^(-q*T) * N(d1)

No imports of scipy — pure math for IMC sandbox compatibility.
"""
