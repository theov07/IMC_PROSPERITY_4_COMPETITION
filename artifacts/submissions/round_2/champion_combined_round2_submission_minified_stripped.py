...
class BaseStrategy(ABC):
	...
	def on_tick(self, state: TradingState, memory: Dict[str, Any]) -> Tuple[List[Order], int]:
		...
		return self.compute_orders(state=state, book=book, order_depth=order_depth, position=position, memory=memory)
	@abstractmethod
	...
	def log_quote_snapshot(self, *, state: TradingState, memory: Dict[str, Any], bid_price: int | float | None, ask_price: int | float | None, extras: Dict[str, Any] | None=None) -> None:
	...
	def log_taker_fill(self, *, state: TradingState, memory: Dict[str, Any], side: str, price: int, quantity: int, gap_exploit: bool=False) -> None:
...
class MMFirstV4ComboStrategy(BaseStrategy):
	def _compute_quote_prices(self, book: BookSnapshot, inventory_ratio: float, mid_smooth: float) -> Tuple[Optional[int], Optional[int], str]:
	...
	def _compute_anchor_signal(self, mid: float, book: BookSnapshot, mid_smooth: float, memory: Dict[str, Any]) -> float:
	...
	def _compute_asym_take_edges(self, base_edge: float, position: int, memory: Dict[str, Any]) -> Tuple[float, float]:
	...
	def _fire_takers(self, order_depth: OrderDepth, fair_value: float, bid_size: float, ask_size: float, buy_cap: int, sell_cap: int, buy_edge: float, sell_edge: float) -> Tuple[List[Order], int, int, Set[int], Set[int]]:
	...
	def _gap_exploit(self, order_depth: OrderDepth, memory: Dict[str, Any], limit: int, bid_size: float, ask_size: float, bid_price: Optional[int], ask_price: Optional[int], buy_cap: int, sell_cap: int, taker_buy_px: Set[int], taker_sell_px: Set[int]) -> Tuple[List[Order], int, int, Optional[int], Optional[int]]:
	...
	def _apply_inventory_bias(self, fair_value: float, position: int, memory: Dict[str, Any]) -> float:
	...
	def _passive_quotes(self, bid_price: Optional[int], ask_price: Optional[int], bid_size: float, ask_size: float, buy_cap: int, sell_cap: int, position: int, limit: int) -> Tuple[List[Order], int, int]:
	...
	def _log_taker_fills(self, state: TradingState, memory: Dict[str, Any], this_taker_buy_px: Set[int], this_taker_sell_px: Set[int]) -> None:
...
class TheoBestCleanGeneralizedStrategy(StrategyBase):
	def _update_regression(self, *, state: TradingState, mid: float, memory: Dict[str, Any]) -> Dict[str, float]:
		...
		if len(completed_means) < min_completed_blocks:
			...
			fit_r2 = 0.0 if ss_tot <= 1e-09 else max(0.0, 1.0 - ss_res / ss_tot)
		...
		stats = {'slope': slope, 'intercept': intercept, 'fitted_now': fitted_now, 'forecast': forecast, 'residual': residual, 'rmse': rmse, 'r2': fit_r2, 'confidence': confidence, 'trend_ticks': trend_ticks, 'fair_value': fair_value, 'residual_z': residual_z, 'block_count': float(len(completed_means)), 'current_block_mean': current_block_mean}
		memory['regression_stats'] = stats
		return stats
	def _inventory_target(self, *, state: TradingState, stats: Dict[str, float], position: int) -> int:
	...
	def _size_from_target(self, *, position: int, inv_target: int, stats: Dict[str, float], buy_cap: int, sell_cap: int) -> Tuple[int, int]:
		...
		bearish_boost = max(0.0, -stats['trend_ticks']) * float(self.params.get('trend_sell_boost_per_tick', 0.2))
	...
	def _process_premium_fills(self, state: TradingState, memory: Dict[str, Any]) -> None:
	...
	def _handle_onesided_book(self, book: BookSnapshot, position: int, memory: Dict[str, Any], gap_sell_quotes: List[int], gap_buy_quotes: List[int]) -> Optional[Tuple[List[Order], int]]:
		orders: List[Order] = []
		...
		if book.best_bid is None and book.best_ask is None:
			...
			gap_buy_price = ref - empty_side_shift_buy
			gap_sell_price = ref + empty_side_shift_sell
		...
		if book.best_bid is None or book.best_ask is None:
			...
			return (orders, 0)
		return None
	def _update_ewma_signals(self, spot: float, fv: float, memory: Dict[str, Any]) -> Tuple[float, float, float, float, float, float]:
	...
	def _compute_regime(self, state: TradingState, stats: Dict[str, float], spot: float, stretch: float, book: BookSnapshot, position: int, memory: Dict[str, Any]) -> Dict[str, Any]:
		...
		gap_rebuy_mode = bullish and last_gap_sell_price is not None and (0 <= gap_rebuy_age <= gap_rebuy_window) and (position < inv_target) and (gap_rebuy_discount >= gap_rebuy_min_discount)
		if gap_rebuy_mode:
			active_build_target = inv_target
		...
		trim_quote_mode = bullish and (not build_phase) and (position >= trim_start_position) and (stretch >= trim_extension_threshold)
		...
		return {'timestamp': ts, 'bullish': bullish, 'build_phase': build_phase, 'inv_target': inv_target, 'active_build_target': active_build_target, 'on_dip': on_dip, 'startup_window_active': startup_window_active, 'startup_fast_loading': startup_fast_loading, 'startup_cold_loading': startup_cold_loading, 'startup_fast_passive_buy': startup_fast_passive_buy, 'startup_cold_passive_buy': startup_cold_passive_buy, 'startup_chase_passive_buy': startup_chase_passive_buy, 'startup_anchor_bid_spread': startup_anchor_bid_spread, 'startup_anchor_gap_ticks': startup_anchor_gap_ticks, 'startup_anchor_size': startup_anchor_size, 'startup_cold_join_ticks': startup_cold_join_ticks, 'pullback_seen': pullback_seen, 'current_pullback_ready': current_pullback_ready, 'build_release_ready': build_release_ready, 'gap_rebuy_mode': gap_rebuy_mode, 'gap_rebuy_discount': gap_rebuy_discount, 'chasing': chasing, 'rebuy_blocked': rebuy_blocked, 'buy_edge': buy_edge, 'buy_take_cap': buy_take_cap, 'trim_quote_mode': trim_quote_mode, 'trim_take_mode': trim_take_mode, 'trim_take_qty': trim_take_qty, 'rich_z': rich_z, 'cheap_z': cheap_z}
	def _compute_quote_prices(self, book: BookSnapshot, fv: float, stats: Dict[str, float], regime: Dict[str, Any], entry_reference: float) -> Tuple[int, int]:
	...
	def _buy_takers(self, order_depth: OrderDepth, fv: float, position: int, buy_cap: int, regime: Dict[str, Any]) -> Tuple[List[Order], int, int, Set[int]]:
		orders: List[Order] = []
	...
	def _sell_takers(self, order_depth: OrderDepth, fv: float, position: int, sell_cap: int, regime: Dict[str, Any]) -> Tuple[List[Order], int, int]:
		orders: List[Order] = []
		pending_sell = 0
		build_phase = regime['build_phase']
	...
	def _compute_passive_sizes(self, position: int, buy_cap: int, sell_cap: int, pending_buy: int, pending_sell: int, stats: Dict[str, float], regime: Dict[str, Any], entry_reference: float, book: BookSnapshot, bid_price: int, ask_price: int, buy_taker_prices: Set[int]) -> Tuple[int, int, Optional[int], int, int, bool]:
		build_phase = regime['build_phase']
		...
		for ap, _ in book.ask_levels:
			if ap not in buy_taker_prices:
				real_best_ask = ap
				break
		buy_size, sell_size = self._size_from_target(position=position + pending_buy - pending_sell, inv_target=inv_target, stats=stats, buy_cap=buy_cap, sell_cap=sell_cap)
	...
	def _gap_trap_quotes(self, book: BookSnapshot, position: int, memory: Dict[str, Any], sell_cap: int, ask_price: int, trend_ticks: float, gap_rebuy_mode: bool) -> Tuple[List[Order], List[int]]:
		orders: List[Order] = []
		gap_sell_prices: List[int] = []
		empty_side_shift = int(self.params.get('empty_side_shift_sell', self.params.get('empty_side_shift', 85)))
		...
		if gap_trap_anchor_ask is not None:
			...
			if candidate_gap_trap_sell > ask_price:
				...
				gap_trap_sell_size = min(sell_cap, gap_trap_base_size, max(0, position - gap_trap_floor_position + 1))
				gap_trap_active = gap_trap_sell_size > 0
			if gap_trap_peak_ask is not None and gap_trap_fragile_streak >= gap_trap_premium_streak and (sell_cap > gap_trap_sell_size):
				candidate_gap_trap_premium = max((gap_trap_sell_price or ask_price) + gap_trap_premium_extra, int(gap_trap_peak_ask) + empty_side_shift + gap_trap_premium_extra)
				if candidate_gap_trap_premium > (gap_trap_sell_price or ask_price):
					gap_trap_premium_price = candidate_gap_trap_premium
					gap_trap_premium_size = min(max(0, sell_cap - gap_trap_sell_size), gap_trap_premium_size_limit, max(0, position - gap_trap_floor_position))
	...
	def compute_orders(self, state: TradingState, book: BookSnapshot, order_depth: OrderDepth, position: int, memory: Dict[str, Any]) -> Tuple[List[Order], int]:
		...
		_, _, _, stretch, trim_reference, entry_reference = self._update_ewma_signals(spot, fv, memory)
		...
		buy_cap = self.buy_capacity(position)
		sell_cap = self.sell_capacity(position)
		buy_orders, buy_cap, pending_buy, swept_ask_prices = self._buy_takers(order_depth, fv, position, buy_cap, regime)
		sell_orders, sell_cap, pending_sell = self._sell_takers(order_depth, fv, position, sell_cap, regime)
		buy_size, sell_size, anchor_buy_price, anchor_buy_size, ask_price, anchor_mode = self._compute_passive_sizes(position, buy_cap, sell_cap, pending_buy, pending_sell, stats, regime, entry_reference, book, bid_price, ask_price, swept_ask_prices)
		gap_trap_orders, gap_trap_sell_prices = self._gap_trap_quotes(book, position, memory, sell_cap, ask_price, stats['trend_ticks'], regime['gap_rebuy_mode'])
		...
		gap_trap_anchor_ask = memory.get('_gap_trap_anchor_ask')
		gap_trap_peak_ask = memory.get('_gap_trap_peak_ask')
		self.log_quote_snapshot(state=state, memory=memory, bid_price=bid_price, ask_price=ask_price, extras={'position': position, 'trend_ticks': round(trend_ticks, 2), 'fair_value': round(fv, 2), 'stretch': round(stretch, 2), 'trim_ref': round(trim_reference, 2), 'entry_ref': round(entry_reference, 2), 'inv_target': regime['inv_target'], 'active_build_target': regime['active_build_target'], 'bullish': int(regime['bullish']), 'build_phase': int(regime['build_phase']), 'on_dip': int(regime['on_dip']), 'chasing': int(regime['chasing']), 'pullback_ready': int(regime['current_pullback_ready']), 'pullback_seen': int(regime['pullback_seen']), 'build_release_ready': int(regime['build_release_ready']), 'gap_rebuy_mode': int(regime['gap_rebuy_mode']), 'gap_rebuy_discount': round(regime['gap_rebuy_discount'], 2), 'anchor_mode': int(anchor_mode), 'startup_fast_loading': int(regime['startup_fast_loading']), 'startup_cold_loading': int(regime['startup_cold_loading']), 'buy_size': buy_size, 'sell_size': sell_size, 'anchor_buy_price': anchor_buy_price, 'anchor_buy_size': anchor_buy_size, 'gap_trap_active': int(gap_trap_active), 'gap_trap_armed': int(gap_trap_armed), 'gap_trap_fragile_streak': gap_trap_fragile_streak, 'gap_trap_anchor_ask': gap_trap_anchor_ask, 'gap_trap_peak_ask': gap_trap_peak_ask, 'trim_quote_mode': int(regime['trim_quote_mode']), 'trim_take_mode': int(regime['trim_take_mode']), 'trim_take_qty': regime['trim_take_qty'], 'rebuy_blocked': int(regime['rebuy_blocked'])})
		return (orders, 0)
	def feature_prices(self, memory: Dict[str, Any]) -> Dict[str, float]:
		out: Dict[str, float] = {}
...
class TheoBestCleanGeneralizedV3Strategy(TheoBestCleanGeneralizedV2Strategy):
	def _max_inventory_sell_guard_active(self, *, position: int, best_ask: Optional[int], fair_value: float) -> bool:
		guard_position = int(self.params.get('max_inventory_sell_guard_position', self.position_limit()))
		guard_threshold = float(self.params.get('max_inventory_sell_guard_threshold', 8.0))
		active = best_ask is not None and position >= guard_position and (float(best_ask) < fair_value + guard_threshold)
		...
		return active
	def _compute_passive_sizes(self, position: int, buy_cap: int, sell_cap: int, pending_buy: int, pending_sell: int, stats, regime, entry_reference: float, book, bid_price: int, ask_price: int, buy_taker_prices: Set[int]) -> Tuple[int, int, Optional[int], int, int, bool]:
		buy_size, sell_size, anchor_buy_price, anchor_buy_size, ask_price, anchor_mode = super()._compute_passive_sizes(position, buy_cap, sell_cap, pending_buy, pending_sell, stats, regime, entry_reference, book, bid_price, ask_price, buy_taker_prices)
		if self._max_inventory_sell_guard_active(position=position, best_ask=book.best_ask, fair_value=float(stats['fair_value'])):
			sell_size = 0
		return (buy_size, sell_size, anchor_buy_price, anchor_buy_size, ask_price, anchor_mode)
class TheoBestCleanGeneralizedV4Strategy(TheoBestCleanGeneralizedV3Strategy):
	...
	def _dump_reserve_release_active(self, *, position: int, best_ask: Optional[int], fair_value: float) -> bool:
		reserve_size = self._reserve_inventory_size()
		reserve_threshold = float(self.params.get('dump_reserve_release_threshold', 3.0))
		reserve_min_position = int(self.params.get('dump_reserve_release_min_position', self._reserve_normal_inventory_cap()))
		active = reserve_size > 0 and best_ask is not None and (position >= reserve_min_position) and (float(best_ask) <= fair_value - reserve_threshold)
		...
		return active
	def _reserve_buy_room(self, *, position: int, pending_buy: int, release_active: bool) -> int:
	...
	def _buy_takers(self, order_depth, fv: float, position: int, buy_cap: int, regime):
		best_ask = min(order_depth.sell_orders) if order_depth.sell_orders else None
		release_active = self._dump_reserve_release_active(position=position, best_ask=best_ask, fair_value=float(fv))
		capped_buy_cap = min(buy_cap, self._reserve_buy_room(position=position, pending_buy=0, release_active=release_active))
		if capped_buy_cap <= 0:
			return ([], 0, 0, set())
	...
	def _compute_passive_sizes(self, position: int, buy_cap: int, sell_cap: int, pending_buy: int, pending_sell: int, stats, regime, entry_reference: float, book, bid_price: int, ask_price: int, buy_taker_prices: Set[int]) -> Tuple[int, int, Optional[int], int, int, bool]:
		release_active = self._dump_reserve_release_active(position=position, best_ask=book.best_ask, fair_value=float(stats['fair_value']))
		capped_buy_cap = min(buy_cap, self._reserve_buy_room(position=position, pending_buy=pending_buy, release_active=release_active))
		return super()._compute_passive_sizes(position, capped_buy_cap, sell_cap, pending_buy, pending_sell, stats, regime, entry_reference, book, bid_price, ask_price, buy_taker_prices)
class TheoBestCleanGeneralizedV5Strategy(TheoBestCleanGeneralizedV4Strategy):
	def _buy_gap_trap_quotes(self, book, position: int, memory: Dict[str, Any], buy_cap: int, active_bid_price: int, trend_ticks: float, gap_rebuy_mode: bool) -> Tuple[List[Order], List[int]]:
		orders: List[Order] = []
		gap_buy_prices: List[int] = []
		if buy_cap <= 0 or book.best_bid is None:
			memory['buy_gap_trap_active'] = 0
			return (orders, gap_buy_prices)
		empty_side_shift = int(self.params.get('empty_side_shift_buy', self.params.get('empty_side_shift', 85)))
		...
		buy_gap_trap_floor_position = int(self.params.get('buy_gap_trap_floor_position', self._reserve_normal_inventory_cap()))
		...
		buy_gap_trap_base_size = int(self.params.get('buy_gap_trap_base_size', max(1, min(2, self._reserve_inventory_size()))))
		buy_gap_trap_premium_size_limit = int(self.params.get('buy_gap_trap_premium_size', max(0, self._reserve_inventory_size() - buy_gap_trap_base_size)))
		...
		if len(book.bid_levels) >= 2:
			bid_gap_fragile = bid_gap_fragile or book.bid_levels[0][0] - book.bid_levels[1][0] >= buy_gap_trap_min_gap
		...
		if bid_side_fragile:
			...
			trap_fragile_bids[:] = []
		trap_armable = trend_ticks >= buy_gap_trap_min_trend and (not gap_rebuy_mode) and (position >= buy_gap_trap_floor_position) and (buy_cap > 0)
		if trap_armable and bid_side_fragile:
			...
			buy_gap_trap_clear_streak = 0
		if buy_gap_trap_anchor_bid is None and trap_armable and (buy_gap_trap_fragile_streak >= buy_gap_trap_arm_streak) and trap_recent_bids:
		...
		if buy_gap_trap_anchor_bid is not None:
			if not trap_armable or buy_gap_trap_clear_streak >= buy_gap_trap_clear_after:
				...
				if trap_fragile_bids:
					...
					buy_gap_trap_trough_bid = min(int(buy_gap_trap_trough_bid or latest_trough), latest_trough)
		...
		if buy_gap_trap_anchor_bid is not None:
			...
			if buy_gap_trap_trough_bid is not None and buy_gap_trap_fragile_streak >= buy_gap_trap_premium_streak and (buy_cap > buy_gap_trap_buy_size):
				candidate_buy_gap_premium = min((buy_gap_trap_buy_price or active_bid_price) - buy_gap_trap_premium_extra, int(buy_gap_trap_trough_bid) - empty_side_shift - buy_gap_trap_premium_extra)
				if candidate_buy_gap_premium < (buy_gap_trap_buy_price or active_bid_price):
					...
					buy_gap_trap_premium_size = min(max(0, buy_cap - buy_gap_trap_buy_size), buy_gap_trap_premium_size_limit)
		...
		return (orders, gap_buy_prices)
	def compute_orders(self, state: TradingState, book, order_depth: OrderDepth, position: int, memory: Dict[str, Any]) -> Tuple[List[Order], int]:
		...
		buy_gap_orders, buy_gap_prices = self._buy_gap_trap_quotes(book=book, position=position, memory=memory, buy_cap=buy_cap_remaining, active_bid_price=active_bid_price, trend_ticks=trend_ticks, gap_rebuy_mode=gap_rebuy_mode)
...
class TheoBestCleanGeneralizedV6Strategy(TheoBestCleanGeneralizedV5Strategy):
	def _apply_startup_price_improvement(self, *, state, stats: Dict[str, float], stretch: float, book, position: int, memory: Dict[str, Any], regime: Dict[str, Any]) -> Dict[str, Any]:
		tuned = dict(regime)
		reserve_normal_cap = self._reserve_normal_inventory_cap()
		improvement_start_position = int(self.params.get('startup_price_improve_start_position', max(0, reserve_normal_cap - 21)))
		...
		improvement_release_threshold = float(self.params.get('startup_price_improve_release_threshold', 1.5))
		...
		improvement_passive_buy_cap = int(self.params.get('startup_price_improve_passive_buy_cap', 4))
		improvement_anchor_extra_spread = float(self.params.get('startup_price_improve_anchor_extra_spread', 1.0))
		improvement_end_ts = int(self.params.get('startup_price_improve_end_ts', self.params.get('startup_delayed_finish_ts', 3000)))
		best_ask = book.best_ask
		ask_richness = 0.0 if best_ask is None else float(best_ask) - float(stats['fair_value'])
		release_ready = tuned['on_dip'] or tuned['current_pullback_ready'] or ask_richness <= improvement_release_threshold or (float(stats['residual_z']) <= 0.0)
		improvement_active = tuned['build_phase'] and tuned['startup_window_active'] and (not tuned['gap_rebuy_mode']) and (int(state.timestamp) <= improvement_end_ts) and (position >= improvement_start_position) and (best_ask is not None) and (ask_richness >= improvement_hot_threshold) and (stretch >= improvement_hot_stretch) and (not release_ready)
		if improvement_active:
			improved_target_cap = max(improvement_start_position, reserve_normal_cap - improvement_holdback)
			tuned['active_build_target'] = min(int(tuned['active_build_target']), improved_target_cap)
			tuned['buy_take_cap'] = min(int(tuned['buy_take_cap']), improvement_take_cap)
			tuned['startup_fast_passive_buy'] = min(int(tuned['startup_fast_passive_buy']), improvement_passive_buy_cap)
			tuned['startup_cold_passive_buy'] = min(int(tuned['startup_cold_passive_buy']), improvement_passive_buy_cap)
		...
		memory['startup_price_improve_cap'] = max(improvement_start_position, reserve_normal_cap - improvement_holdback) if improvement_active else int(tuned['active_build_target'])
		return tuned
	def _compute_regime(self, state, stats: Dict[str, float], spot: float, stretch: float, book, position: int, memory: Dict[str, Any]) -> Dict[str, Any]:
		regime = super()._compute_regime(state, stats, spot, stretch, book, position, memory)
		return self._apply_startup_price_improvement(state=state, stats=stats, stretch=stretch, book=book, position=position, memory=memory, regime=regime)
class TheoBestCleanGeneralizedV7Strategy(TheoBestCleanGeneralizedV6Strategy):
	def _apply_startup_recent_low_patience(self, *, state, stats: Dict[str, float], stretch: float, book, position: int, memory: Dict[str, Any], regime: Dict[str, Any]) -> Dict[str, Any]:
		tuned = dict(regime)
		best_ask = book.best_ask
		...
		is_crash = best_ask is not None and prev_best_ask is not None and (float(best_ask) <= float(prev_best_ask) - crash_drop_ticks)
		if best_ask is not None:
		...
		if recent_low is None or best_ask is None:
			...
			return tuned
		reserve_normal_cap = self._reserve_normal_inventory_cap()
		patience_start_position = int(self.params.get('startup_recent_low_start_position', max(0, reserve_normal_cap - 29)))
		...
		patience_anchor_extra_spread = float(self.params.get('startup_recent_low_anchor_extra_spread', 1.0))
		patience_buy_edge_floor = float(self.params.get('startup_recent_low_buy_edge_floor', 1.0))
		patience_end_ts = int(self.params.get('startup_recent_low_end_ts', self.params.get('startup_delayed_finish_ts', 3000)))
		recent_gap = float(best_ask) - float(recent_low)
		release_ready = tuned['on_dip'] or tuned['current_pullback_ready'] or recent_gap <= patience_release_gap or (float(stats['residual_z']) <= 0.0) or is_crash
		patience_active = tuned['build_phase'] and tuned['startup_window_active'] and (not tuned['gap_rebuy_mode']) and (int(state.timestamp) <= patience_end_ts) and (position >= patience_start_position) and (recent_gap >= patience_gap_threshold) and (stretch >= patience_hot_stretch) and (not release_ready)
		if patience_active:
			...
			tuned['startup_fast_passive_buy'] = min(int(tuned['startup_fast_passive_buy']), patience_passive_buy_cap)
			tuned['startup_cold_passive_buy'] = min(int(tuned['startup_cold_passive_buy']), patience_passive_buy_cap)
		...
		memory['v7_fix_crash_detected'] = int(is_crash)
		return tuned
	def _compute_regime(self, state, stats: Dict[str, float], spot: float, stretch: float, book, position: int, memory: Dict[str, Any]) -> Dict[str, Any]:
		regime = super()._compute_regime(state, stats, spot, stretch, book, position, memory)
		return self._apply_startup_recent_low_patience(state=state, stats=stats, stretch=stretch, book=book, position=position, memory=memory, regime=regime)
	def compute_orders(self, state, book, order_depth, position: int, memory: Dict[str, Any]):
		orders, conversions = super().compute_orders(state, book, order_depth, position, memory)
		...
		return (orders, conversions)
PRODUCTS = {'ASH_COATED_OSMIUM': {'OB_cleared_shift': 89, 'anchor_alpha': 0.02, 'anchor_drift_bound': 2.0, 'anchor_price': 10000.0, 'ar_gain': 0.3, 'ar_shift_source': 'mid_smooth', 'gap_trigger_confirm_ticks': 1, 'gap_trigger_max_vol_pct': 0.1, 'gap_trigger_min': 10, 'inventory_aversion_gamma': 0.0015, 'last_ts_value': 999900, 'log_flush_ts': 1000, 'maker_size': 20, 'maker_size_base_pct': 0.5, 'mid_smooth_half_life': 10, 'mid_smooth_window': 50, 'pct_kept_for_takers': 0.05, 'position_limit': 80, 'quote_trace_enabled': True, 'strategy': 'mm_first_v4_combo', 'take_edge': 1, 'take_edge_hi': 0.8, 'take_edge_lo': 0.3, 'take_edge_vol_hi': 5.0, 'take_edge_vol_lo': 2.0, 'taker_buy_threshold': 9990, 'taker_sell_threshold': 10025, 'tighten_ticks': 1, 'ts_increment': 100, 'unwind_take_edge': 3.0, 'zscore_gap_gate': 1.5, 'zscore_max_scale': 5.0, 'zscore_size_scale': 0.5, 'zscore_threshold': 1, 'zscore_window': 50}, 'INTARIAN_PEPPER_ROOT': {'aggravate_cut': 0.04, 'ask_gap_quote_size': 160, 'ask_gap_sell_enable_position': 0, 'ask_spread_bull': 9.0, 'bid_spread_bull': 1.0, 'block_size': 200, 'bootstrap_confidence': 0.55, 'bull_threshold': 1.0, 'buy_gap_trap_arm_streak': 2, 'buy_gap_trap_base_size': 2, 'buy_gap_trap_clear_after': 4, 'buy_gap_trap_floor_position': 77, 'buy_gap_trap_fragile_bid_window': 6, 'buy_gap_trap_max_imbalance': 0.05, 'buy_gap_trap_min_gap': 3, 'buy_gap_trap_min_trend': 0.0, 'buy_gap_trap_premium_extra': 2, 'buy_gap_trap_premium_size': 1, 'buy_gap_trap_premium_streak': 2, 'buy_gap_trap_recent_bid_window': 12, 'buy_gap_trap_top_bid_max': 12, 'chase_threshold': 1.25, 'cheap_buy_boost_per_z': 0.18, 'cheap_residual_z': 0.9, 'dip_threshold': 1.0, 'dump_reserve_inventory': 6, 'dump_reserve_release_min_position': 74, 'dump_reserve_release_threshold': 3.0, 'empty_side_shift': 85, 'empty_side_shift_buy': 85, 'empty_side_shift_sell': 89, 'fastfill_buy_edge_boost': 0.0, 'fastfill_deep_take_guard_end_ts': 1000, 'fastfill_deep_take_max_gap_ticks': 1, 'fastfill_end_ts': 12000, 'fastfill_min_passive_buy': 10, 'fastfill_target': 80, 'fv_alpha': 0.05, 'gap_fill_min_premium': 35, 'gap_rebuy_buy_edge': -10.0, 'gap_rebuy_min_discount': 20.0, 'gap_rebuy_passive_buy': 6, 'gap_rebuy_take_cap': 8, 'gap_rebuy_window': 2500, 'gap_trap_arm_streak': 2, 'gap_trap_base_size': 4, 'gap_trap_clear_after': 4, 'gap_trap_floor_position': 75, 'gap_trap_fragile_ask_window': 6, 'gap_trap_min_gap': 3, 'gap_trap_min_imbalance': -0.05, 'gap_trap_min_trend': 0.0, 'gap_trap_premium_extra': 2, 'gap_trap_premium_size': 3, 'gap_trap_premium_streak': 2, 'gap_trap_recent_ask_window': 12, 'gap_trap_top_ask_max': 12, 'hold_sell_offset': 0, 'hold_sell_size': 0, 'last_ts_value': 999900, 'log_flush_ts': 1000, 'maker_size': 80, 'max_bid_extra_ticks': 2, 'max_inventory_sell_guard_position': 80, 'max_inventory_sell_guard_threshold': 0.0, 'min_completed_blocks': 5, 'neut_spread_ask': 5.0, 'neut_spread_bid': 2.0, 'one_sided_target_gap': 24, 'position_limit': 80, 'rebuy_block_ticks': 25, 'reg_horizon': 25, 'reg_r2_cap': 0.98, 'reg_r2_floor': 0.85, 'reg_residual_reversion': 0.25, 'reg_rmse_floor': 1.0, 'resid_inv_per_z': 14.0, 'rich_residual_z': 1.0, 'rich_sell_boost_per_z': 0.14, 'seed_slope': 0.1015, 'short_alpha': 0.22, 'slope_window': 20, 'standing_deep_qty': 15, 'startup_anchor_bid_spread': 1.0, 'startup_anchor_gap_ticks': 1, 'startup_anchor_size': 4, 'startup_chase_passive_buy': 1, 'startup_chase_take_cap': 1, 'startup_chase_take_edge': 4.0, 'startup_cold_join_ticks': 0, 'startup_cold_passive_buy': 3, 'startup_cold_take_cap': 4, 'startup_cold_take_edge': 3.0, 'startup_delayed_finish_ts': 3000, 'startup_dip_take_edge_boost': 1.0, 'startup_end_ts': 30000, 'startup_fast_passive_buy': 8, 'startup_fast_take_cap': 12, 'startup_fast_target': 64, 'startup_post_pullback_target': 72, 'startup_pre_pullback_target': 48, 'startup_price_improve_anchor_extra_spread': 1.0, 'startup_price_improve_end_ts': 3000, 'startup_price_improve_holdback': 8, 'startup_price_improve_hot_stretch': 0.75, 'startup_price_improve_hot_threshold': 4.0, 'startup_price_improve_passive_buy_cap': 4, 'startup_price_improve_release_threshold': 1.5, 'startup_price_improve_start_position': 56, 'startup_price_improve_take_cap': 3, 'startup_pullback_ticks': 2.0, 'startup_recent_low_anchor_extra_spread': 1.0, 'startup_recent_low_buy_edge_floor': 1.0, 'startup_recent_low_crash_drop_ticks': 3.0, 'startup_recent_low_end_ts': 3000, 'startup_recent_low_gap_threshold': 2.0, 'startup_recent_low_holdback': 6, 'startup_recent_low_hot_stretch': 0.4, 'startup_recent_low_passive_buy_cap': 3, 'startup_recent_low_release_gap': 0.5, 'startup_recent_low_start_position': 48, 'startup_recent_low_take_cap': 2, 'startup_recent_low_window': 12, 'startup_release_stretch': 1.0, 'startup_release_take_cap': 8, 'startup_target': 80, 'strategy': 'theo_best_clean_generalized_v7', 'strong_trend_ticks': 0.9, 'take_buy_edge_bull': -8.0, 'take_buy_edge_neut': 2.0, 'take_sell_edge_neut': 2.0, 'target_gap_scale': 26.0, 'tighten_ticks': 1, 'trend_buy_boost_per_tick': 0.24, 'trend_inv_per_tick': 16.0, 'trend_inventory_cap': 80, 'trend_sell_boost_per_tick': 0.2, 'trim_ask_local_edge': 0.0, 'trim_cooldown_ticks': 20, 'trim_extension_threshold': 0.75, 'trim_floor_position': 78, 'trim_reference_slope_weight': 0.15, 'trim_sell_size': 1, 'trim_signal_edge': 1.0, 'trim_start_position': 79, 'trim_take_edge': 2.0, 'trim_take_position': 80, 'trim_take_sell_size': 1, 'trim_take_stretch': 999.0, 'ts_increment': 100, 'unwind_take_edge': 10.0, 'very_strong_trend_ticks': 1.6}}
