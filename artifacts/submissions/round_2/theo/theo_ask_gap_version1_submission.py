"""Theo ask-gap version 1 for INTARIAN_PEPPER_ROOT.

Philosophy:
  - Core: stay max-long on PEPPER_ROOT (linear trend ~+1000 pts/day).
    Regression-based fair value drives passive bid/ask quotes and taker buys.
  - Startup: fast-fill to position limit in the first 12000 ticks using
    sub-phases (fast/cold/chase/anchor) calibrated to the opening ramp.
  - Gap trap: when the ask side is fragile for N consecutive ticks and
    position is at the floor, arm a passive SELL at anchor_ask + empty_side_shift.
    Fills when a market-taker sweep clears the book to that price.
  - Gap rebuy: after a confirmed gap-sell fill, re-enter aggressively
    at a discount during the post-spike pullback window.
  - One-sided book: when best_ask is None, post a wide SELL at
    last_best_ask + empty_side_shift (immediate-fill trap).

Key params:
  seed_slope              — fallback regression slope before enough data (default 0.1015)
  fastfill_target         — target position during build phase (default 80)
  fastfill_end_ts         — timestamp cutoff for forced build phase (default 12000)
  empty_side_shift        — tick offset for one-sided-book quotes (default 85)
  gap_trap_arm_streak     — consecutive fragile ticks before arming trap (default 2)
  gap_trap_floor_position — minimum position to arm the gap trap (default 80)
  gap_trap_base_size      — base size for gap trap sell order (default 4)
  gap_rebuy_window        — ticks after gap sell during which rebuy is active (default 2500)
  gap_rebuy_min_discount  — min discount vs gap sell price to trigger rebuy (default 20.0)
"""
