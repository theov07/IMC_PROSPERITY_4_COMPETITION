...
# ── INTARIAN_PEPPER_ROOT Strategy — theo_tester_round_2 ──────────────────────────
# Generic one-sided-book gap exploiter (the mechanism behind v2/v4/v6/v7 PnL spikes):
#   1. Build 80 long fast (PEPPER_ROOT trends +1000/day ≈ +80K carry per full day held)
#   2. Hard one-sided book (best_bid or best_ask None) → aggressive far order, capped
#   3. Ask-side fragile streak + pos high → arm gap-sell at last_best_ask + empty_side_shift
#   4. Bid-side fragile streak + pos not max → arm gap-buy at last_best_bid - empty_side_shift
#   5. After any gap-sell fill → gap_rebuy_mode: aggressively rebuild position at the new cheap price
#   6. Cap gap-sell at 5 units (v2 fix — don't dump trend carry position)

class TestTheoStrategy(Round1RegressionMMV5Strategy):
    """Full Leo fusion for buys + v34 trim sells + symmetric bid/ask fragility gap scout."""
    ...
    def compute_orders(
        ...
        # Cap IPR gap sells to avoid wiping out the trend position.
        base_gap_sell_cap = int(self.params.get("gap_sell_cap", 5))
        gap_sell_cap = base_gap_sell_cap
        recent_ask_fragile_streak = int(memory.get("_ask_fragile_streak", 0))
        if recent_ask_fragile_streak >= int(self.params.get("gap_sell_cap_fragile_streak", 2)):
            gap_sell_cap = min(
                int(self.params.get("gap_sell_cap_high", base_gap_sell_cap)),
                base_gap_sell_cap + max(0, recent_ask_fragile_streak - int(self.params.get("gap_sell_cap_fragile_streak", 2)) + 1),
            )
        ...
        if book.best_bid is None and book.best_ask is None:
            if last_best_bid is None and last_best_ask is None:
                return orders, 0
            # Use last_best_ask as reference for sell (higher price = more premium)
            sell_ref = last_best_ask if last_best_ask is not None else last_best_bid
            buy_ref = last_best_bid if last_best_bid is not None else last_best_ask
            gap_buy_price = buy_ref - empty_side_shift
            gap_sell_price = sell_ref + empty_side_shift
            orders.append(Order(self.product, gap_buy_price, self.buy_capacity(position)))
            # v2 fix: cap sell at gap_sell_cap (default 5) to preserve trend position
            orders.append(Order(self.product, gap_sell_price, -min(gap_sell_cap, self.sell_capacity(position))))
            ...
            # Use last_best_ask as reference for gap sell price
        ...
        gap_scout_sell_price = None
        gap_scout_sell_size = 0
        gap_scout_reach_sell_price = None
        gap_scout_reach_sell_size = 0
        gap_scout_active = False
        ask_side_fragile = len(book.ask_levels) == 1
        gap_scout_outside_window = False
        gap_scout_fragile_streak = 0
        if len(book.ask_levels) >= 2:
            gap_scout_min_gap = int(self.params.get("gap_scout_min_gap", 3))
            ask_side_fragile = ask_side_fragile or (book.ask_levels[1][0] - book.ask_levels[0][0] >= gap_scout_min_gap)

        prev_fragile_streak = int(memory.get("_ask_fragile_streak", 0))
        if bullish and ask_side_fragile:
            gap_scout_fragile_streak = prev_fragile_streak + 1
        else:
            gap_scout_fragile_streak = 0
        memory["_ask_fragile_streak"] = gap_scout_fragile_streak

        recent_fragile_asks = memory.setdefault("_recent_fragile_asks", [])
        fragile_ask_window = int(self.params.get("gap_scout_fragile_ask_window", 4))
        if bullish and ask_side_fragile:
            recent_fragile_asks.append(int(book.best_ask))
            if len(recent_fragile_asks) > fragile_ask_window:
                del recent_fragile_asks[:-fragile_ask_window]
        else:
            recent_fragile_asks[:] = []

        if bullish and position >= int(self.params.get("gap_scout_floor_position", 75)):
            gap_scout_base_size_limit = int(self.params.get("gap_scout_base_size_limit", 7))
            gap_scout_reach_size_limit = int(self.params.get("gap_scout_reach_size_limit", 3))
            gap_scout_always_active = bool(self.params.get("gap_scout_always_active", False))
            gap_scout_arm_streak = int(self.params.get("gap_scout_arm_streak", 2))
            gap_scout_reach_arm_streak = int(self.params.get("gap_scout_reach_arm_streak", 3))
            gap_scout_min_trend = float(self.params.get("gap_scout_min_trend", 1.15))
            gap_scout_outside_position_buffer = int(self.params.get("gap_scout_outside_position_buffer", 2))
            gap_scout_reach_min_extra = int(self.params.get("gap_scout_reach_min_extra", 2))
            gap_scout_early_start_ts = int(self.params.get("gap_scout_early_start_ts", 3600))
            gap_scout_early_end_ts = int(self.params.get("gap_scout_early_end_ts", 8500))
            gap_scout_mid_start_ts = int(self.params.get("gap_scout_mid_start_ts", 56500))
            gap_scout_mid_end_ts = int(self.params.get("gap_scout_mid_end_ts", 57500))
            gap_scout_late_start_ts = int(self.params.get("gap_scout_late_start_ts", 143000))
            gap_scout_late_end_ts = int(self.params.get("gap_scout_late_end_ts", 145000))

            in_priority_gap_window = (
                gap_scout_early_start_ts <= int(state.timestamp) <= gap_scout_early_end_ts
                or gap_scout_mid_start_ts <= int(state.timestamp) <= gap_scout_mid_end_ts
                or gap_scout_late_start_ts <= int(state.timestamp) <= gap_scout_late_end_ts
            )
            outside_window_armed = (
                gap_scout_always_active
                and gap_scout_fragile_streak >= gap_scout_arm_streak
                and trend_ticks >= gap_scout_min_trend
                and position >= int(self.params.get("gap_scout_floor_position", 75)) + gap_scout_outside_position_buffer
            )
            gap_scout_outside_window = outside_window_armed and not in_priority_gap_window

            recent_best_asks = memory.get("_recent_best_asks", [])
            if (in_priority_gap_window or outside_window_armed) and ask_side_fragile and recent_best_asks:
                gap_scout_anchor_ask = min(recent_best_asks)
                candidate_gap_scout_sell = gap_scout_anchor_ask + empty_side_shift
                if candidate_gap_scout_sell > ask_price:
                    gap_scout_sell_price = candidate_gap_scout_sell
                    gap_scout_sell_size = min(
                        sell_cap,
                        gap_scout_base_size_limit,
                        max(0, position - int(self.params.get("gap_scout_floor_position", 75)) + 1),
                    )
                    gap_scout_active = gap_scout_sell_size > 0

                if recent_fragile_asks and gap_scout_fragile_streak >= gap_scout_reach_arm_streak and sell_cap > gap_scout_sell_size:
                    reach_anchor_ask = max(recent_fragile_asks)
                    candidate_reach_sell = reach_anchor_ask + empty_side_shift
                    if candidate_reach_sell >= (gap_scout_sell_price or ask_price) + gap_scout_reach_min_extra:
                        gap_scout_reach_sell_price = candidate_reach_sell
                        gap_scout_reach_sell_size = min(
                            max(0, sell_cap - gap_scout_sell_size),
                            gap_scout_reach_size_limit,
                            max(0, position - int(self.params.get("gap_scout_floor_position", 75))),
                        )
                        gap_scout_active = gap_scout_active or gap_scout_reach_sell_size > 0

        # ── Symmetric bid-side gap scout ──
        # Mirror of the ask-side scout: when the bid side is fragile (1 level, or gap to L2 >= min_gap)
        # for a streak of ticks, arm an aggressive BUY far below at last_best_bid - empty_side_shift.
        # Value: captures the v6-type event where market-sell flow sweeps through our deep bid,
        # loading inventory at ~85 ticks below mid in a product that trends +0.1/tick → huge carry.
        # Only armed when we have buy capacity (position below ceiling).
        gap_buy_scout_price = None
        gap_buy_scout_size = 0
        bid_side_fragile = len(book.bid_levels) == 1
        if len(book.bid_levels) >= 2:
            gap_scout_min_gap_p = int(self.params.get("gap_scout_min_gap", 3))
            bid_side_fragile = bid_side_fragile or (book.bid_levels[0][0] - book.bid_levels[1][0] >= gap_scout_min_gap_p)

        prev_bid_fragile_streak = int(memory.get("_bid_fragile_streak", 0))
        if bullish and bid_side_fragile:
            bid_fragile_streak = prev_bid_fragile_streak + 1
        else:
            bid_fragile_streak = 0
        memory["_bid_fragile_streak"] = bid_fragile_streak

        recent_fragile_bids = memory.setdefault("_recent_fragile_bids", [])
        fragile_bid_window = int(self.params.get("gap_scout_fragile_ask_window", 4))
        if bullish and bid_side_fragile:
            recent_fragile_bids.append(int(book.best_bid))
            if len(recent_fragile_bids) > fragile_bid_window:
                del recent_fragile_bids[:-fragile_bid_window]
        else:
            recent_fragile_bids[:] = []

        gap_buy_scout_enabled = bool(self.params.get("gap_buy_scout_enabled", True))
        gap_buy_scout_ceiling = int(self.params.get("gap_buy_scout_ceiling_position", 76))
        gap_buy_scout_arm_streak = int(self.params.get("gap_buy_scout_arm_streak", 2))
        gap_buy_scout_size_limit = int(self.params.get("gap_buy_scout_size_limit", 6))

        # gap_buy_scout consumes the buy-capacity left over after passive buy_size + anchor, so
        # it never causes position-limit clipping or cannibalises the build-phase passive bid.
        remaining_buy_cap = max(0, buy_cap - buy_size - anchor_buy_size)
        if (
            gap_buy_scout_enabled
            and bullish
            and position <= gap_buy_scout_ceiling
            and bid_fragile_streak >= gap_buy_scout_arm_streak
            and recent_fragile_bids
            and remaining_buy_cap > 0
        ):
            gap_buy_anchor_bid = max(recent_fragile_bids)
            candidate_gap_buy = gap_buy_anchor_bid - empty_side_shift
            if candidate_gap_buy < bid_price and candidate_gap_buy >= 1:
                gap_buy_scout_price = candidate_gap_buy
                gap_buy_scout_size = min(
                    remaining_buy_cap,
                    gap_buy_scout_size_limit,
                    max(0, gap_buy_scout_ceiling - position + 1),
                )
        ...
        if gap_buy_scout_size > 0 and gap_buy_scout_price is not None:
            orders.append(Order(self.product, gap_buy_scout_price, gap_buy_scout_size))
        if sell_size > 0:
            orders.append(Order(self.product, ask_price, -sell_size))
        if gap_scout_sell_size > 0 and gap_scout_sell_price is not None:
            orders.append(Order(self.product, gap_scout_sell_price, -gap_scout_sell_size))
        if gap_scout_reach_sell_size > 0 and gap_scout_reach_sell_price is not None:
            orders.append(Order(self.product, gap_scout_reach_sell_price, -gap_scout_reach_sell_size))
        ...
        memory["gap_scout_active"] = int(gap_scout_active)
        memory["gap_scout_fragile_streak"] = gap_scout_fragile_streak
        memory["gap_scout_outside_window"] = int(gap_scout_outside_window)
        ...
        self.log_quote_snapshot(
            ...
            extras={
                ...
                "gap_scout_active": int(gap_scout_active),
                "gap_scout_fragile_streak": gap_scout_fragile_streak,
                "gap_scout_outside_window": int(gap_scout_outside_window),
                "gap_scout_sell_price": gap_scout_sell_price,
                "gap_scout_sell_size": gap_scout_sell_size,
                "gap_scout_reach_sell_price": gap_scout_reach_sell_price,
                "gap_scout_reach_sell_size": gap_scout_reach_sell_size,
...
PRODUCTS = {
    'INTARIAN_PEPPER_ROOT': {
        ...
        # ── Hold sell (passive sell when at max) ──
        'hold_sell_size': 1,
        'hold_sell_offset': 0,
        ...
        # ── Gap scout: keep known windows, allow outside-window activation only once fragility confirms
        'gap_scout_always_active': True,
        'gap_scout_arm_streak': 2,
        'gap_scout_base_size_limit': 7,
        'gap_scout_early_end_ts': 8500,
        'gap_scout_early_start_ts': 3600,
        'gap_scout_fragile_ask_window': 4,
        'gap_scout_floor_position': 75,
        'gap_scout_late_end_ts': 145000,
        'gap_scout_late_start_ts': 143000,
        'gap_scout_mid_end_ts': 57500,
        'gap_scout_mid_start_ts': 56500,
        'gap_scout_min_gap': 3,
        'gap_scout_min_trend': 1.15,
        'gap_scout_outside_position_buffer': 2,
        'gap_scout_reach_arm_streak': 3,
        'gap_scout_reach_min_extra': 2,
        'gap_scout_reach_size_limit': 3,
        'gap_scout_recent_ask_window': 6,
        'gap_scout_size_limit': 10,
        'empty_side_shift': 85,
        'gap_sell_cap': 5,
        'gap_sell_cap_fragile_streak': 2,
        'gap_sell_cap_high': 7,
        # ── Symmetric bid-side gap scout (captures v6-type buy-side gap events) ──
        'gap_buy_scout_enabled': True,
        'gap_buy_scout_ceiling_position': 76,
        'gap_buy_scout_arm_streak': 2,
        'gap_buy_scout_size_limit': 6,
    },
}

STRATEGY_CLASSES = {
    "test_theo": TestTheoStrategy,
}
