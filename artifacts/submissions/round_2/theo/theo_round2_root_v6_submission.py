...
class TestTheoStrategy(Round1RegressionMMV5Strategy):
    ...
    def compute_orders(
        ...
        if book.best_bid is None and book.best_ask is None:
            ...
            orders.append(Order(self.product, ref - empty_side_shift, self.buy_capacity(position)))
            orders.append(Order(self.product, ref + empty_side_shift, -self.sell_capacity(position)))
            return orders, 0
        ...
        pre_gap_trap_price = None
        pre_gap_trap_size = 0
        if bullish and not build_phase:
            pre_gap_trap_start_ts = int(self.params.get("pre_gap_trap_start_ts", 15000))
            pre_gap_trap_floor_position = int(self.params.get("pre_gap_trap_floor_position", 79))
            pre_gap_trap_size_limit = int(self.params.get("pre_gap_trap_size_limit", 4))
            pre_gap_trap_min_gap = int(self.params.get("pre_gap_trap_min_gap", 3))

            ask_side_fragile = len(book.ask_levels) == 1
            if len(book.ask_levels) >= 2:
                ask_side_fragile = ask_side_fragile or (book.ask_levels[1][0] - book.ask_levels[0][0] >= pre_gap_trap_min_gap)

            if (
                int(state.timestamp) >= pre_gap_trap_start_ts
                and position >= pre_gap_trap_floor_position
                and ask_side_fragile
            ):
                ref_candidates = [price for price in (last_best_ask, book.best_ask) if price is not None]
                if ref_candidates:
                    ref_ask = min(ref_candidates)
                    pre_gap_trap_price = ref_ask + empty_side_shift
                    pre_gap_trap_size = min(
                        sell_cap,
                        pre_gap_trap_size_limit,
                        max(0, position - pre_gap_trap_floor_position + 1),
                    )
        ...
        if pre_gap_trap_size > 0 and pre_gap_trap_price is not None:
            orders.append(Order(self.product, pre_gap_trap_price, -pre_gap_trap_size))
        ...
        self.log_quote_snapshot(
            ...
            extras={
                ...
                "pre_gap_trap_price": pre_gap_trap_price,
                "pre_gap_trap_size": pre_gap_trap_size,
...
PRODUCTS = {'INTARIAN_PEPPER_ROOT': {
    ...
    'startup_target': 80,
    'pre_gap_trap_floor_position': 79,
    'pre_gap_trap_min_gap': 3,
    'pre_gap_trap_size_limit': 4,
    'pre_gap_trap_start_ts': 15000,
    'strategy': 'test_theo',
