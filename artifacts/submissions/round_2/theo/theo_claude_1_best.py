...
# ── INTARIAN_PEPPER_ROOT Strategy (v7 logic, unchanged) ──────────────────────────
...
class TestTheoStrategy(Round1RegressionMMV5Strategy):
    ...
    def compute_orders(
        ...
        if bullish and position >= int(self.params.get("gap_scout_floor_position", 78)):
            gap_scout_min_gap = int(self.params.get("gap_scout_min_gap", 3))
            gap_scout_size_limit = int(self.params.get("gap_scout_size_limit", 5))
            ...
            if in_gap_window and ask_side_fragile and recent_best_asks:
                ...
                if candidate_gap_scout_sell > ask_price:
                    ...
                    gap_scout_sell_size = min(
                        sell_cap,
                        gap_scout_size_limit,
                        max(0, position - int(self.params.get("gap_scout_floor_position", 78)) + 1),
                    )
...
class AshCoatedOsmiumMMStrategy(BaseStrategy):
    """
    Inventory-adaptive market maker for ACO (mean-reverting ~10000).

    Penny-improves on both sides, reduces size on inventory-adverse side.
    Stays flat directionally to avoid trend risk on a non-trending asset.
    """
    ...
    def compute_orders(
        ...
        # If both sides empty, post gap orders to catch extreme fills
        if book.best_bid is None and book.best_ask is None:
            ...
            buy_cap = self.buy_capacity(position)
            sell_cap = self.sell_capacity(position)
            gap_size = int(self.params.get("gap_size", 3))
            if buy_cap > 0:
        ...
        # Inventory-adaptive sizing: lean away from existing position
        ...
        # Hard stops: don't buy if already very long, don't sell if very short
        if position >= max_pos:
            bid_size = 0
        if position <= min_pos:
            ask_size = 0
        ...
        # Penny-improve: post inside the spread
...
PRODUCTS = {
    'INTARIAN_PEPPER_ROOT': {
        ...
        # ── Gap scout (extended windows vs v7) ──
        'gap_scout_early_end_ts': 8500,    # was 5200 in v7, now extended to catch more early fragile moments
        'gap_scout_early_start_ts': 3600,
        'gap_scout_floor_position': 78,
        'gap_scout_late_end_ts': 145000,   # new third window
        'gap_scout_late_start_ts': 143000, # new third window
        'gap_scout_mid_end_ts': 57500,
        'gap_scout_mid_start_ts': 56500,
        'gap_scout_min_gap': 3,
        'gap_scout_recent_ask_window': 6,
        'gap_scout_size_limit': 7,         # was 5 in v7, increased to capture more premium
        'empty_side_shift': 85,
    },
    'ASH_COATED_OSMIUM': {
        'strategy': 'aco_mm',
        ...
        # Gap fills when both sides empty
        'empty_side_shift': 85,
        'gap_size': 3,
    },
}
...
STRATEGY_CLASSES = {
    ...
    "aco_mm": AshCoatedOsmiumMMStrategy,
}
...
class Trader:
    ...
    def bid(self) -> int:
        # MAF bid: raised from v7's 15 to 500 for much higher chance of being
        # in the top-50% of bidders and gaining 25% extra market access flow.
        # Extra flow = more passive fill opportunities at gap_scout prices.
        return 500
