...
class TestTheoStrategy(Round1RegressionMMV5Strategy):
    def compute_orders(
        ...
        orders: List[Order] = []
        ob_shift = int(self.params.get("ob_cleared_shift", 0))

        # Track last known bests for OB-cleared shift logic
        if book.best_bid is not None:
            memory["_last_best_bid"] = book.best_bid
        if book.best_ask is not None:
            memory["_last_best_ask"] = book.best_ask

        # OB cleared: one or both sides empty — post wide quotes to catch NPC sweeps
        # Safety: only post if WE didn't send takers last tick (avoid self-created opportunities)
        we_took_last_tick = memory.get("_we_took_last_tick", False)
        if book.best_bid is None or book.best_ask is None:
            if ob_shift > 0 and not we_took_last_tick:
                last_bid = memory.get("_last_best_bid")
                last_ask = memory.get("_last_best_ask")
                if last_bid is None and book.best_ask is not None:
                    last_bid = book.best_ask - 1
                if last_ask is None and book.best_bid is not None:
                    last_ask = book.best_bid + 1
                buy_cap = self.buy_capacity(position)
                sell_cap = self.sell_capacity(position)
                if book.best_bid is None and buy_cap > 0 and last_bid is not None:
                    orders.append(Order(self.product, last_bid - ob_shift, buy_cap))
                if book.best_ask is None and sell_cap > 0 and last_ask is not None:
                    orders.append(Order(self.product, last_ask + ob_shift, -sell_cap))
            memory["_we_took_last_tick"] = False
            return orders, 0
        ...
        # ── Short-term EMA for stretch detection (from v34) ──
        ...
        # ── Standard Leo fusion quote prices ──
        ...
        # ── Trim detection: use STRETCH (mid - short_ema) directly ──
        # trim_quote_mode: post a passive sell inside the spread when extended
        trim_quote_mode = (
            bullish
            and not build_phase
            and position >= trim_start_position
            and stretch >= trim_stretch_threshold
        )

        # trim_take_mode: DISABLED — taker sells at best_bid lose to the 14-tick
        # spread on INTARIAN_PEPPER_ROOT. Only passive sells are used.
        trim_take_mode = False

        # rebuy_blocked only applies to taker trims (disabled in this version)
        ...
        # ── Taker orders (sell side) — only when NOT in build phase and NOT in trim mode ──
        pending_sell = 0
        if not build_phase and not trim_quote_mode:
        ...
        # Track if we sent takers this tick (for OB-shift safety guard)
        memory["_we_took_last_tick"] = (pending_buy > 0 or pending_sell > 0)

        # ── Trim take: sell at best_bid on big stretch spikes ──
        trim_take_qty = 0
        if trim_take_mode:
            trim_take_qty = min(
                sell_cap,
                max(0, position - trim_floor_position),
                max(1, trim_take_sell_size),
            )
            if trim_take_qty > 0:
        ...
        # ── Passive sizing (Leo fusion logic) ──
        ...
        # ── In trim_quote_mode: post passive sell at best_ask - 1 (penny improve) ──
        # Selling inside the spread gives better queue priority and more fills.
        # If filled at best_ask - 1, we capture (best_ask-1 - rebuy_price) per unit.
        # If NOT filled, we stay at position 80 and lose nothing.
        if trim_quote_mode and not build_phase:
            ...
            # Penny-improve the ask (best_ask - 1), but stay above mid
            trim_ask = max(book.best_bid + 1, book.best_ask - 1, round(mid + 5))
            ask_price = trim_ask

        # ── After trim fill: rebuy is NOT blocked — we want to get back to 80 fast ──
        # The rebuy_block is only used for taker trims (disabled).
        # For passive trim fills, we immediately try to rebuy.
