...
class TheoVraiStratStrategy(Round1RegressionMMV5Strategy):
    """test_theo base + maker fallback when the book becomes one-sided or empty."""
    ...
    def compute_orders(
        ...
        # Inspired by mm_first_v2: if the effective book is one-sided (no mid_price),
        # anchor wide maker quotes around the last seen market bests rather than doing
        # nothing. We only do this when the opportunity is already present in the book.
        if book.mid_price is None:
            if last_best_bid is None and book.best_ask is not None:
                last_best_bid = book.best_ask - 1
            if last_best_ask is None and book.best_bid is not None:
                last_best_ask = book.best_bid + 1

            buy_cap = self.buy_capacity(position)
            sell_cap = self.sell_capacity(position)

            if buy_cap > 0 and last_best_bid is not None:
                orders.append(Order(self.product, last_best_bid - empty_side_shift, buy_cap))
            if sell_cap > 0 and last_best_ask is not None:
                orders.append(Order(self.product, last_best_ask + empty_side_shift, -sell_cap))
            return orders, 0
...
STRATEGY_CLASSES = {"theo_vrai_strat": TheoVraiStratStrategy}
