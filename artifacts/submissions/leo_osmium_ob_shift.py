...
class NaiveTightMarketMakerV10Strategy(BaseStrategy):
    ...
    def compute_orders(
        ...
        if book.best_bid is not None and book.best_ask is not None:
            ...
            mid = float(book.best_bid if book.best_bid is not None else book.best_ask)
        ...
        # When one side is empty, only quote the side that exists
        if real_best_bid is None or real_best_ask is None:
            bid_price = (real_best_bid + tighten_ticks) if real_best_bid is not None else None
            ask_price = (real_best_ask - tighten_ticks) if real_best_ask is not None else None
        else:
        ...
        bid_jumped = bool(prev_best_bid is not None and real_best_bid is not None and real_best_bid == prev_best_bid + 1)
        ask_jumped = bool(prev_best_ask is not None and real_best_ask is not None and real_best_ask == prev_best_ask - 1)
        ...
        # ── Emit passive orders ────────────────────────────────────────
        if buy_size > 0 and bid_price is not None:
            orders.append(Order(self.product, bid_price, buy_size))
        if sell_size > 0 and ask_price is not None:
...
class OsmiumMeanRevStrategy(NaiveTightMarketMakerV10Strategy):
    ...
    def compute_orders(
        ...
        # Track last known bests for OB-cleared shift
        if book.best_bid is not None:
            memory["_last_best_bid"] = book.best_bid
        if book.best_ask is not None:
            memory["_last_best_ask"] = book.best_ask

        # OB cleared: post wide when book empty on one or both sides (Tibo's idea)
        # Instead of returning early, we inject OB shift orders and let the rest
        ...
        if book.best_bid is None or book.best_ask is None:
            ...
            if book.best_bid is None and buy_cap > 0 and last_bid is not None:
                ob_shift_orders.append(Order(self.product, last_bid - ob_shift, buy_cap))
            if book.best_ask is None and sell_cap > 0 and last_ask is not None:
                ob_shift_orders.append(Order(self.product, last_ask + ob_shift, -sell_cap))

            # Both sides empty: can't do normal MM, just return OB shift orders
            if book.best_bid is None and book.best_ask is None:
                return ob_shift_orders, 0

        mid = (book.best_bid + book.best_ask) / 2.0 if (book.best_bid is not None and book.best_ask is not None) else float(book.best_bid or book.best_ask)
        ...
        try:
            parent_orders, conversions = super().compute_orders(state, book, order_depth, position, memory)
        finally:
            if anchor_alpha == 0.0:
                self.params["anchor_price"] = fixed_anchor

        # Prepend OB shift orders (priority) to parent's normal orders
        return ob_shift_orders + parent_orders, conversions
