...
class R3HydroReversionMMStrategy(BaseStrategy):
    def compute_orders(
        ...
        ema = self._update_ema(mid, memory)
        deviation = mid - ema
        ...
        bid_size, ask_size = self._quote_sizes(position, deviation)
        ...
        take_order = self._take_order(state, book, position, deviation, memory, buy_cap, sell_cap)
    ...
    def _update_ema(self, mid: float, memory: Dict[str, Any]) -> float:
        alpha = float(self.params.get("ema_alpha", 0.008))
        ema = memory.get("ema")
        if ema is None:
            return mid
        return alpha * mid + (1.0 - alpha) * float(ema)
    ...
    def _quote_sizes(self, position: int, deviation: float) -> tuple[int, int]:
    ...
    def _take_order(
        ...
        if deviation > threshold and position > -soft_limit and sell_cap > 0:
        ...
        if deviation < -threshold and position < soft_limit and buy_cap > 0:
