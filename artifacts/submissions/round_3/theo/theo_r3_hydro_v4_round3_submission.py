...
class R3HydroReversionMMStrategy(BaseStrategy):
    def compute_orders(
        ...
        take_order = self._take_order(state, book, position, deviation, trend, memory, buy_cap, sell_cap)
    ...
    def _quote_sizes(self, position: int, deviation: float, trend: float) -> tuple[int, int]:
        ...
        bid_size = maker
        ask_size = maker
        # Mean-reversion signal: only fire when trend is not reinforcing the same direction.
        # trend < -trend_guard means fast EMA is well below slow EMA (downtrend) → suppress buy cut.
        # trend > +trend_guard means fast EMA is well above slow EMA (uptrend) → suppress sell cut.
        if deviation > quote_threshold and trend < trend_guard:
            bid_size = 0
            ask_size = maker + min(signal_boost, int(abs(deviation) // 4))
        elif deviation < -quote_threshold and trend > -trend_guard:
            ask_size = 0
    ...
    def _take_order(
        ...
        soft_limit = int(self.params.get("soft_position_limit", 40))
        last_take_ts = int(memory.get("last_take_ts", -10**9))

        if int(state.timestamp) - last_take_ts < cooldown_ts:
            return None

        # Same trend guard: only take if trend is not reinforcing the signal direction
        if deviation > threshold and trend < trend_guard and position > -soft_limit and sell_cap > 0:
            qty = min(take_size, sell_cap, soft_limit + position)
            if qty > 0:
                memory["last_take_ts"] = int(state.timestamp)
                return Order(self.product, int(book.best_bid), -qty)

        if deviation < -threshold and trend > -trend_guard and position < soft_limit and buy_cap > 0:
            qty = min(take_size, buy_cap, soft_limit - position)
            if qty > 0:
                memory["last_take_ts"] = int(state.timestamp)
...
# ── prosperity/strategies/round_3/theo/theo_r3_vol_arb_v1.py ──────────────────────
...
PRODUCTS = {'HYDROGEL_PACK': {'ema_alpha': 0.008,
                   ...
                   'trend_guard': 8.0,
                   'ts_increment': 100},
 'VELVETFRUIT_EXTRACT': {'hedge_abs_position_limit': 140,
...
STRATEGY_CLASSES = {"option_mm_bs": OptionMMBSStrategy, "r3_hydro_reversion_mm": R3HydroReversionMMStrategy, "theo_r3_vol_arb_v1": TheoR3VolArbV1Strategy}
