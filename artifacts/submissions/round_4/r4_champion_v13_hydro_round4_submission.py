...
class HydroMVV13(BaseStrategy):
    ...
    def _update_m14_hydro_gate(self, state, memory):
        ...
        return cum, gated
    def _update_vev_hedge_gate(self, state, memory):
        ...
        return sig, gated
    def _position_gates(self, position):
        limit = self.position_limit()
    ...
    def _ar_takers(self, order_depth, fair_value, sell_allowed, buy_allowed, sell_gated, buy_cap, sell_cap):
    ...
    def _mm_passive(self, book, position, sell_allowed, sell_gated, buy_allowed, buy_cap, sell_cap):
        limit = self.position_limit()
    ...
    def compute_orders(self, state, book, order_depth, position, memory):
        ...
        sell_gated = m14_gated or vev_gated
        buy_cap = self.buy_capacity(position)
        sell_cap = self.sell_capacity(position)
        taker_orders, buy_cap, sell_cap, t_bought, t_sold = self._ar_takers(
            order_depth, fair_value, sell_allowed, buy_allowed, sell_gated, buy_cap, sell_cap)
        mm_orders, mm_bid_qty, mm_ask_qty = self._mm_passive(
            book, position, sell_allowed, sell_gated, buy_allowed, buy_cap, sell_cap)
        self.log_quote_snapshot(state=state, memory=memory, bid_price=None, ask_price=None,
            extras={'position': position, 'mid': round(float(mid), 2), 'FairValue': round(fair_value, 2),
                    'Anchor': round(anchor_val, 2), 'DevSmooth': round(dev, 3), 'ar_mom': round(ar_mom, 4),
                    'm14_hydro_cum': m14_cum, 'vev_hedge_sig': round(vev_sig, 2),
                    'm14_gated': int(m14_gated), 'vev_gated': int(vev_gated),
                    'sell_gated': int(sell_gated), 'sell_allowed': int(sell_allowed),
                    'taker_buy': t_bought, 'taker_sell': t_sold,
                    'mm_bid_qty': mm_bid_qty, 'mm_ask_qty': mm_ask_qty})
...
PRODUCTS = {
    'HYDROGEL_PACK': {'anchor_price': 10000, 'anchor_alpha': 0.005, 'anchor_pos_threshold': 0.30, 'ar_gain': 8.0, 'ar_smooth_half_life': 5, 'mid_smooth_half_life': 20, 'dev_smooth_half_life': 5, 'ar_taker_edge': 12.0, 'ar_taker_size_pct': 0.30, 'sell_cap_pct': 1.0, 'buy_cap_pct': 1.0, 'maker_size_base_pct': 0.12, 'inv_skew_ticks': 4, 'm14_trader': 'Mark 14', 'm14_hydro_threshold': 75.0, 'vev_gate_product': 'VEV_4000', 'vev_gate_trader': 'Mark 14', 'vev_gate_hl': 100.0, 'vev_gate_threshold': 5.0, 'last_ts_value': 999900, 'quote_trace_enabled': True, 'log_flush_ts': 1000, 'strategy': 'hydro_mv_v13', 'position_limit': 200},
...
STRATEGY_CLASSES = {'gamma_scalp_zgated': GammaScalpZGatedStrategy, 'r3_guarded_anchor_mm': R3GuardedAnchorMMStrategy, 'vev_option_mm_v3': VEVOptionMMV3Strategy, 'hydro_mv_v13': HydroMVV13}
class Trader:
