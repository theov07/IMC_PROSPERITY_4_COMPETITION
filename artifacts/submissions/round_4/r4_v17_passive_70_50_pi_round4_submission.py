...
class BaseStrategy(ABC):
    ...
    def on_tick(
        ...
        orders = self._apply_cross_asset_bias(state, position, orders, book, memory)
        orders = self._apply_inventory_unwind(state, position, orders, book, memory)
        return orders, conversions
    def _apply_cross_asset_bias(self, state, position, orders, book, memory):
        p = self.params
        if not bool(p.get("cross_asset_enabled", False)) or not orders:
            return orders
        src_sym = p.get("cross_asset_source_symbol", "")
        src_trader = p.get("cross_asset_source_trader", "")
        if not src_sym or not src_trader:
            return orders
        ts_now = int(getattr(state, "timestamp", 0))
        buf = memory.setdefault("_xa_buf", [])
        for t in ((state.market_trades or {}).get(src_sym) or []):
            qty = float(getattr(t, "quantity", 0))
            if qty <= 0:
                continue
            if getattr(t, "buyer", "") == src_trader:
                buf.append([ts_now, qty])
            elif getattr(t, "seller", "") == src_trader:
                buf.append([ts_now, -qty])
        cutoff = ts_now - int(p.get("cross_asset_window_ts", 10000))
        while buf and buf[0][0] < cutoff:
            buf.pop(0)
        signal = float(p.get("cross_asset_weight", 0.0)) * sum(q for _, q in buf)
        if abs(signal) <= float(p.get("cross_asset_threshold", 5.0)):
            return orders
        max_off = float(p.get("cross_asset_max_offset", 2.0))
        offset = int(round(max(-max_off, min(max_off, signal * float(p.get("cross_asset_scale", 0.05))))))
        if offset == 0:
            return orders
        bb, ba = book.best_bid, book.best_ask
        shifted = []
        for o in orders:
            np_ = int(o.price) + offset
