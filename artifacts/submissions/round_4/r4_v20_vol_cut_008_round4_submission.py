...
class BaseStrategy(ABC):
    ...
    def _apply_cross_asset_bias(self, state, position, orders, book, memory):
        ...
        if not src_sym or not src_trader:
            return orders
        weight = float(p.get("cross_asset_weight", 0.0))
        window_ts = int(p.get("cross_asset_window_ts", 10000))
        ts_now = int(getattr(state, "timestamp", 0))
        buf = memory.setdefault("_xa_buf", [])
        try:
            mt = state.market_trades or {}
            for t in (mt.get(src_sym) or []):
        ...
        if buf and buf[0][0] < cutoff:
            ...
            while i < len(buf) and buf[i][0] < cutoff:
                i += 1
            del buf[:i]
        net = sum(q for _, q in buf)
        signal = weight * net
        thresh = float(p.get("cross_asset_threshold", 5.0))
        if abs(signal) <= thresh:
            return orders
        max_off = float(p.get("cross_asset_max_offset", 2.0))
        scale = float(p.get("cross_asset_scale", 0.05))
        offset = int(round(max(-max_off, min(max_off, signal * scale))))
        if offset == 0:
            return orders
