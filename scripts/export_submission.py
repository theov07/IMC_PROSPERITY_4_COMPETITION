...
Flattens the strategy dispatcher + all needed strategy code into one file
that can be uploaded to the Prosperity platform.
...
Usage:
  ...
  python scripts/export_submission.py --member leo_naive --round 0 --output my_naive_submission.py
"""
...
# The template includes ALL strategy types inline so any product config works.
TEMPLATE = dedent('''\
import json
...
def main() -> int:
    ...
    parser.add_argument("--member", default="champion", choices=["champion", "leo", "leo_naive", "theo", "pietro"])
    ...
    # Convert ProductConfig objects to plain dicts for embedding
    products = {}
    ...
    rendered = TEMPLATE.replace("__PRODUCTS__", pformat(products, width=100))

    output_path = Path(args.output or f"artifacts/submissions/{args.member}_round{args.round}_submission.py")
