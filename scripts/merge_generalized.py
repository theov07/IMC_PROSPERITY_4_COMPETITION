"""Merge Tibo OSMIUM generalized + Theo ROOT generalized into one submission."""

with open('artifacts/submissions/tibo_osmium_generalized.py', 'r', encoding='utf-8') as f:
    tibo = f.read().split('\n')
with open('artifacts/submissions/theo_root_generalized.py', 'r', encoding='utf-8') as f:
    theo = f.read().split('\n')

# Part 1: Shared infra from Tibo (imports + BookSnapshot + persistence + BaseStrategy) lines 1-408
shared = '\n'.join(tibo[:408])

# Part 2: Tibo's MMFirstStrategy (line 409 to line before config)
tibo_strat = '\n'.join(tibo[408:875])

# Part 3: Theo's strategies (Round1RegressionMMV5Strategy + _ewma + TestTheoStrategy)
# From line 299 (comment before class) to line before config (832)
theo_strat = '\n'.join(theo[298:833])

# Part 4: Combined config
osmium_config = '\n'.join(tibo[877:906])  # PRODUCTS dict for OSMIUM (without closing }})
ipr_config = '\n'.join(theo[835:901])     # PRODUCTS dict for IPR

config = f"""
# ── Config ────────────────────────────────────────────────────────────────────

PRODUCTS = {{
    'ASH_COATED_OSMIUM': {{'OB_cleared_shift': 75,
...
out_path = 'artifacts/submissions/tibo_theo_generalized.py'
