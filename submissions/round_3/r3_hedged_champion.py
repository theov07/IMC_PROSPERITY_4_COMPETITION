"""Backtester entrypoint — r3_hedged_champion (option MM + delta-hedged underlying)."""
...
class Trader:
    def __init__(self):
        config = get_round_config(3, "r3_hedged_champion")
    ...
    def run(self, state: TradingState):
        ...
        convs = 0
        # Run option strategies FIRST so they publish positions before the
        # hedger reads them. The dict key order is insertion order — the
        # coordinator doesn't depend on strict ordering, but running the
        # hedger last gives it the freshest option positions this tick.
