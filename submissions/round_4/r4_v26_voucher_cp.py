...
class Trader:
    ...
    def __init__(self):
        velvet_cfg = PRODUCTS['VELVETFRUIT_EXTRACT']
        params = {k: v for k, v in velvet_cfg.items() if k != 'strategy'}
        cls = STRATEGY_CLASSES[velvet_cfg['strategy']]
        self.velvet_strategy = cls(product='VELVETFRUIT_EXTRACT', params=params)
    def bid(self):
        return 15
    @staticmethod
    ...
    def run(self, state):
        ...
        if 'HYDROGEL_PACK' in ods:
            ...
            target = self._signal(ema_H, self.REFERENCE_H, self.HYD_MR_THR, self.HYD_MR_EXIT, self.HYD_MR_SIZE, cur)
            bid, ask = self._bid_ask(ods['HYDROGEL_PACK'])
            orders = self._orders_to_target('HYDROGEL_PACK', cur, target, bid, ask)
            if orders:
        ...
        # === v26: cp_bias on far-OTM vouchers in EXIT ZONE (|dev_S| < 1) ===
        # When TraderBSV is flat (no directional bet), use Mark 01 (informed buyer) +0.3
        # and Mark 22 (noise seller) -0.3 to take small directional positions.
        # Also enables VEV_6000/6500 (which TraderBSV ignores entirely).
        VOUCHER_CP_KS = [5300, 5400, 5500, 6000, 6500]
        VOUCHER_CP_SIZE = 50
        VOUCHER_CP_WINDOW_TS = 10000
        VOUCHER_CP_THRESHOLD = 5.0
        VOUCHER_CP_WEIGHTS = {'Mark 01': +0.3, 'Mark 22': -0.3}
        in_exit_zone = abs(ema_S - self.REFERENCE_S) < 1.0
        ts_now = int(getattr(state, 'timestamp', 0))
        cp_bufs = saved.setdefault('cp_voucher', {})
        for K in VOUCHER_CP_KS:
            sym = f'VEV_{K}'
            if sym not in ods:
                continue
            buf = cp_bufs.setdefault(sym, [])
            try:
                mt = (state.market_trades or {}).get(sym) or []
            except Exception:
                mt = []
            for t in mt:
                qty = float(getattr(t, 'quantity', 0))
                ...
                if seller:
                    buf.append([ts_now, seller, -qty])
            cutoff = ts_now - VOUCHER_CP_WINDOW_TS
            while buf and buf[0][0] < cutoff:
                buf.pop(0)
            if not in_exit_zone:
                # TraderBSV is directional → don't add cp_bias overlay
                # But for K=6000/6500 (not handled by TraderBSV), still apply
                if K not in (6000, 6500):
                    continue
            # Compute signal
            signal = 0.0
            for ts_e, trader, signed_q in buf:
                w = VOUCHER_CP_WEIGHTS.get(trader, 0.0)
                signal += w * signed_q
            if abs(signal) <= VOUCHER_CP_THRESHOLD:
                continue
            cur = positions.get(sym, 0)
            target_dir = +VOUCHER_CP_SIZE if signal > 0 else -VOUCHER_CP_SIZE
            # In exit zone: small target. For 6000/6500: same small target.
            new_target = target_dir
            bid, ask = self._bid_ask(ods[sym])
            orders_cp = self._orders_to_target(sym, cur, new_target, bid, ask)
            if orders_cp:
                # Override existing voucher orders for this symbol if any
                result[sym] = orders_cp
