"""r5_v12_basket_aware — basket-aware MM with SharedR5Context injection.

Injects a single SharedR5Context per tick into each product's memory dict
so basket_aware_mm strategies can read group z-scores etc.
"""
...
class Trader:
    def __init__(self):
        config = get_round_config(5, "r5_v12_basket_aware")
    ...
    def run(self, state: TradingState):
        ...
        for product, strat in self.strategies.items():
            ...
            mem["_ctx"] = ctx  # inject shared context (not serialized)
            orders, c = strat.on_tick(state, mem)
            mem.pop("_ctx", None)  # don't serialize the live ctx ref
